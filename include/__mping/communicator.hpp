...
#include "xxxxx-1ing/group.hpp"
#include "xxxxx-1ing/mpi_constants.hpp"
#include "xxxxx-1ing/mpi_datatype.hpp"
#include "xxxxx-1ing/mpi_ops.hpp"
#include "xxxxx-1ing/named_parameters.hpp"
#include "xxxxx-1ing/rank_ranges.hpp"

namespace xxxxx-1ing {

// Needed by the plugin system to check if a plugin provides a callback function for MPI errors.
xxxxx-1_MAKE_HAS_MEMBER(mpi_error_handler)

/// @brief Wrapper for MPI communicator providing access to \c rank() and \c size() of the communicator. The \ref
/// Communicator is also access point to all MPI communications provided by xxxxx-1.
/// @tparam DefaultContainerType The default container type to use for containers created by xxxxx-1. Defaults to
/// std::vector.
/// @tparam Plugins Plugins adding functionality to xxxxx-1. Plugins should be classes taking a ``Communicator``
/// template parameter and can assume that they are castable to `Communicator` from which they can
/// call any function of `xxxxx-1ing::Communicator`. See `test/plugin_tests.cpp` for examples.
template <
...
/// @brief A basic xxxxx-1 Communicator that uses std::vector when creating new buffers.
using BasicCommunicator = Communicator<>;

/// @brief Gets a \c const reference to a \ref BasicCommunicator for \c MPI_COMM_WORLD.
///
/// Useful if you want access to xxxxx-1's base functionality without keeping an instance of \ref Communicator or
