...
public:
    ...
    /// to \c xxxxx-1ing::statuses(ignore<>).
    /// @return If \p statuses is an owning out parameter, returns the status information, otherwise returns nothing.
    template <typename StatusesParamObjectType = decltype(xxxxx-1ing::statuses(ignore<>))>
    auto wait_all(StatusesParamObjectType statuses_param = xxxxx-1ing::statuses(ignore<>)) {
        static_assert(
    ...
    template <typename StatusesParamObjectType = decltype(xxxxx-1ing::statuses(ignore<>))>
    auto test_all(StatusesParamObjectType statuses_param = xxxxx-1ing::statuses(ignore<>)) {
        static_assert(
            StatusesParamObjectType::parameter_type == internal::ParameterType::statuses,
            "Only statuses parameters are allowed."
        );
    ...
    /// is written. Defaults to \c xxxxx-1ing::status(ignore<>).
    ...
    auto wait_any(StatusParamObjectType status_param = xxxxx-1ing::status(ignore<>)) {
        static_assert(
    ...
    /// @see wait_any
    template <typename StatusParamObjectType = decltype(status(ignore<>))>
    auto test_any(StatusParamObjectType status_param = xxxxx-1ing::status(ignore<>)) {
        static_assert(
            StatusParamObjectType::parameter_type == internal::ParameterType::status,
            "Only status parameters are allowed."
        );
