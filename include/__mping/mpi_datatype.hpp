...
#include "xxxxx-1ing/builtin_types.hpp"
...
struct struct_type {
    static_assert(
        internal::is_std_pair<T>::value || internal::is_std_tuple<T>::value
            || pfr::is_implicitly_reflectable<T, xxxxx-1ing_tag>::value,
...
/// };
/// namespace xxxxx-1ing {
/// // using xxxxx-1's built-in struct serializer
/// template <>
...
/// @tparam T The type to translate into an MPI_Datatype.
template <typename T>
[[nodiscard]] MPI_Datatype mpi_datatype() xxxxx-1_NOEXCEPT {
    static_assert(
        has_static_type_v<T>,
        "\n --> Type not supported directly by xxxxx-1. Please provide a specialization for mpi_type_traits."
    );
