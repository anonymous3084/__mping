...
/// `xxxxx-1ing::internal::ParameterType::send_buf`, only pass `send_buf` to this macro.
#define xxxxx-1_REQUIRED_PARAMETERS(...) , ##__VA_ARGS__

/// @brief Wrapper to pass (possibly empty) list of parameter type names as optional parameters to \c
/// xxxxx-1_CHECK_PARAMETERS.
/// Note that this macro only takes the *name* of parameter types, i.e., instead of using
/// `xxxxx-1ing::internal::ParameterType::send_buf`, only pass `send_buf` to this macro.
#define xxxxx-1_OPTIONAL_PARAMETERS(...) , ##__VA_ARGS__
...
/// r-value references.
///
/// The macro *only* expects the parameter type, i.e., a member name of the `xxxxx-1ing::internal::ParameterType` enum
...
/// ```
/// checks that the parameter pack `Args` contains members of type `xxxxx-1ing::internal::ParameterType::send_buf` and type
/// `xxxxx-1ing::internal::ParameterType::recv_buf`.
///
...
/// @param required A list of required parameter type names wrapped in a xxxxx-1_REQUIRED_PARAMETERS macro.
/// @param optional A list of optional parameter type names wrapped in a xxxxx-1_OPTIONAL_PARAMETERS macro.
#define xxxxx-1_CHECK_PARAMETERS(args, required, optional)                                                          \
    do {                                                                                                            \
        xxxxx-1_PARAMETER_CHECK_HPP_ASSERT_REQUIRED_PARAMETERS(args, required);                                     \
                                                                                                                    \
        using required_parameters_types = typename xxxxx-1ing::internal::parameter_types_to_integral_constants<        \
            xxxxx-1_PARAMETER_CHECK_HPP_PREFIX_PARAMETERS(required)>::type;                                         \
        using optional_parameters_types = typename xxxxx-1ing::internal::parameter_types_to_integral_constants<        \
            xxxxx-1_PARAMETER_CHECK_HPP_PREFIX_PARAMETERS(optional)>::type;                                         \
        using parameter_types = typename xxxxx-1ing::internal::parameters_to_integral_constant<args...>::type;         \
        static_assert(                                                                                              \
            xxxxx-1ing::internal::                                                                                     \
            ...
            "parameters " xxxxx-1_PARAMETER_CHECK_HPP_EVAL_STRINGIFY(required                                       \
            ) " and optional parameters " xxxxx-1_PARAMETER_CHECK_HPP_EVAL_STRINGIFY(optional)                      \
        );                                                                                                          \
        static_assert(xxxxx-1ing::internal::all_unique_v<parameter_types>, "There are duplicate parameter types.");    \
    } while (false)

/// @cond IMPLEMENTATION

// Used to stringify variadic parameters:
// xxxxx-1_PARAMETER_CHECK_HPP_EVAL_STRINGIFY(a, b, c) returns the string "a, b, c"
#define xxxxx-1_PARAMETER_CHECK_HPP_EVAL_STRINGIFY(ignore, ...) "[" #__VA_ARGS__ "]"

// In the following, we implement variadic macros that do something for each of their arguments:
// - xxxxx-1_PARAMETER_CHECK_HPP_PREFIX_PARAMETERS(...) prepends each argument by "xxxxx-1ing::internal::ParameterType::"
// - xxxxx-1_PARAMETER_CHECK_HPP_ASSERT_REQUIRED_PARAMETERS(args, ...) generates a static assert for each of its
...
//
// Since xxxxx-1_REQUIRED_PARAMETERS and xxxxx-1_OPTIONAL_PARAMETERS always resolve to at least one argument (see
// description above), this is sufficient.

// DISPATCH helper macro as described above
#define xxxxx-1_PARAMETER_CHECK_HPP_SELECT10(x1, x2, x3, x4, x5, x6, x7, x8, x9, x10, y, ...) y

// Adds the prefix "xxxxx-1ing::internal::ParameterType::" to each of its arguments (up to 10 arguments)
// I.e., turns "send_buf, recv_buf" into "xxxxx-1ing::internal::ParameterType::send_buf,
// xxxxx-1ing::internal::ParameterType::recv_buf"
//
// We do this because we need both versions of the parameter types: to print nice error messages, we only need the names
// of the types without preceding `xxxxx-1ing::internal::ParameterType::`; to implement the checks, we need the actual
...
// xxxxx-1_PARAMETER_CHECK_HPP_SELECT10 may not be empty.
#define xxxxx-1_PARAMETER_CHECK_HPP_PREFIX_PARAMETERS(...) \
    xxxxx-1_PARAMETER_CHECK_HPP_SELECT10(                  \
        __VA_ARGS__,                                       \
        xxxxx-1_PARAMETER_CHECK_HPP_PREFIX9(__VA_ARGS__),  \
        xxxxx-1_PARAMETER_CHECK_HPP_PREFIX8(__VA_ARGS__),  \
        xxxxx-1_PARAMETER_CHECK_HPP_PREFIX7(__VA_ARGS__),  \
        xxxxx-1_PARAMETER_CHECK_HPP_PREFIX6(__VA_ARGS__),  \
        xxxxx-1_PARAMETER_CHECK_HPP_PREFIX5(__VA_ARGS__),  \
        xxxxx-1_PARAMETER_CHECK_HPP_PREFIX4(__VA_ARGS__),  \
        xxxxx-1_PARAMETER_CHECK_HPP_PREFIX3(__VA_ARGS__),  \
        xxxxx-1_PARAMETER_CHECK_HPP_PREFIX2(__VA_ARGS__),  \
        xxxxx-1_PARAMETER_CHECK_HPP_PREFIX1(__VA_ARGS__),  \
        xxxxx-1_PARAMETER_CHECK_HPP_PREFIX0(__VA_ARGS__),  \
        ignore                                             \
    )

#define xxxxx-1_PARAMETER_CHECK_HPP_PREFIX0(ignore)
#define xxxxx-1_PARAMETER_CHECK_HPP_PREFIX1(ignore, x1) xxxxx-1ing::internal::ParameterType::x1
#define xxxxx-1_PARAMETER_CHECK_HPP_PREFIX2(ignore, x1, x2) \
    xxxxx-1_PARAMETER_CHECK_HPP_PREFIX1(ignore, x1), xxxxx-1_PARAMETER_CHECK_HPP_PREFIX1(ignore, x2)
#define xxxxx-1_PARAMETER_CHECK_HPP_PREFIX3(ignore, x1, x2, x3) \
    xxxxx-1_PARAMETER_CHECK_HPP_PREFIX2(ignore, x1, x2), xxxxx-1_PARAMETER_CHECK_HPP_PREFIX1(ignore, x3)
#define xxxxx-1_PARAMETER_CHECK_HPP_PREFIX4(ignore, x1, x2, x3, x4) \
    xxxxx-1_PARAMETER_CHECK_HPP_PREFIX3(ignore, x1, x2, x3), xxxxx-1_PARAMETER_CHECK_HPP_PREFIX1(ignore, x4)
#define xxxxx-1_PARAMETER_CHECK_HPP_PREFIX5(ignore, x1, x2, x3, x4, x5) \
    xxxxx-1_PARAMETER_CHECK_HPP_PREFIX4(ignore, x1, x2, x3, x4), xxxxx-1_PARAMETER_CHECK_HPP_PREFIX1(ignore, x5)
#define xxxxx-1_PARAMETER_CHECK_HPP_PREFIX6(ignore, x1, x2, x3, x4, x5, x6) \
    xxxxx-1_PARAMETER_CHECK_HPP_PREFIX5(ignore, x1, x2, x3, x4, x5), xxxxx-1_PARAMETER_CHECK_HPP_PREFIX1(ignore, x6)
#define xxxxx-1_PARAMETER_CHECK_HPP_PREFIX7(ignore, x1, x2, x3, x4, x5, x6, x7) \
    xxxxx-1_PARAMETER_CHECK_HPP_PREFIX6(ignore, x1, x2, x3, x4, x5, x6), xxxxx-1_PARAMETER_CHECK_HPP_PREFIX1(ignore, x7)
#define xxxxx-1_PARAMETER_CHECK_HPP_PREFIX8(ignore, x1, x2, x3, x4, x5, x6, x7, x8) \
    xxxxx-1_PARAMETER_CHECK_HPP_PREFIX7(ignore, x1, x2, x3, x4, x5, x6, x7),        \
        xxxxx-1_PARAMETER_CHECK_HPP_PREFIX1(ignore, x8)
#define xxxxx-1_PARAMETER_CHECK_HPP_PREFIX9(ignore, x1, x2, x3, x4, x5, x6, x7, x8, x9) \
    xxxxx-1_PARAMETER_CHECK_HPP_PREFIX8(ignore, x1, x2, x3, x4, x5, x6, x7, x8),        \
        xxxxx-1_PARAMETER_CHECK_HPP_PREFIX1(ignore, x9)

// Generate code that checks that each of the given parameter types are present in args...
// Usage: xxxxx-1_PARAMETER_CHECK_HPP_ASSERT_REQUIRED_PARAMETERS(Args, send_buf, recv_buf)
// Checks that Args... has parameters for xxxxx-1ing::internal::ParameterType::send_buf and
// xxxxx-1ing::internal::ParameterType::recv_buf
//
// Note that the "ignore" argument in this macro is required because the "..." parameter of
// xxxxx-1_PARAMETER_CHECK_HPP_SELECT10 may not be empty.
#define xxxxx-1_PARAMETER_CHECK_HPP_ASSERT_REQUIRED_PARAMETERS(args, ...)          \
    xxxxx-1_PARAMETER_CHECK_HPP_SELECT10(                                          \
        __VA_ARGS__,                                                               \
        xxxxx-1_PARAMETER_CHECK_HPP_ASSERT_REQUIRED_PARAMETER9(args, __VA_ARGS__), \
        xxxxx-1_PARAMETER_CHECK_HPP_ASSERT_REQUIRED_PARAMETER8(args, __VA_ARGS__), \
        xxxxx-1_PARAMETER_CHECK_HPP_ASSERT_REQUIRED_PARAMETER7(args, __VA_ARGS__), \
        xxxxx-1_PARAMETER_CHECK_HPP_ASSERT_REQUIRED_PARAMETER6(args, __VA_ARGS__), \
        xxxxx-1_PARAMETER_CHECK_HPP_ASSERT_REQUIRED_PARAMETER5(args, __VA_ARGS__), \
        xxxxx-1_PARAMETER_CHECK_HPP_ASSERT_REQUIRED_PARAMETER4(args, __VA_ARGS__), \
        xxxxx-1_PARAMETER_CHECK_HPP_ASSERT_REQUIRED_PARAMETER3(args, __VA_ARGS__), \
        xxxxx-1_PARAMETER_CHECK_HPP_ASSERT_REQUIRED_PARAMETER2(args, __VA_ARGS__), \
        xxxxx-1_PARAMETER_CHECK_HPP_ASSERT_REQUIRED_PARAMETER1(args, __VA_ARGS__), \
        xxxxx-1_PARAMETER_CHECK_HPP_ASSERT_REQUIRED_PARAMETER0(args, __VA_ARGS__), \
        ignore                                                                     \
    )

#define xxxxx-1_PARAMETER_CHECK_HPP_ASSERT_REQUIRED_PARAMETER0(args, ignore)
#define xxxxx-1_PARAMETER_CHECK_HPP_ASSERT_REQUIRED_PARAMETER1(args, ignore, x1)                \
    static_assert(                                                                              \
        xxxxx-1ing::internal::has_parameter_type<xxxxx-1ing::internal::ParameterType::x1, args...>(), \
        "Missing required parameter " #x1                                                       \
    );
#define xxxxx-1_PARAMETER_CHECK_HPP_ASSERT_REQUIRED_PARAMETER2(args, ignore, x1, x2) \
    xxxxx-1_PARAMETER_CHECK_HPP_ASSERT_REQUIRED_PARAMETER1(args, ignore, x1);        \
    xxxxx-1_PARAMETER_CHECK_HPP_ASSERT_REQUIRED_PARAMETER1(args, ignore, x2)
#define xxxxx-1_PARAMETER_CHECK_HPP_ASSERT_REQUIRED_PARAMETER3(args, ignore, x1, x2, x3) \
    xxxxx-1_PARAMETER_CHECK_HPP_ASSERT_REQUIRED_PARAMETER2(args, ignore, x1, x2);        \
    xxxxx-1_PARAMETER_CHECK_HPP_ASSERT_REQUIRED_PARAMETER1(args, ignore, x3)
#define xxxxx-1_PARAMETER_CHECK_HPP_ASSERT_REQUIRED_PARAMETER4(args, ignore, x1, x2, x3, x4) \
    xxxxx-1_PARAMETER_CHECK_HPP_ASSERT_REQUIRED_PARAMETER3(args, ignore, x1, x2, x3);        \
    xxxxx-1_PARAMETER_CHECK_HPP_ASSERT_REQUIRED_PARAMETER1(args, ignore, x4)
#define xxxxx-1_PARAMETER_CHECK_HPP_ASSERT_REQUIRED_PARAMETER5(args, ignore, x1, x2, x3, x4, x5) \
    xxxxx-1_PARAMETER_CHECK_HPP_ASSERT_REQUIRED_PARAMETER4(args, ignore, x1, x2, x3, x4);        \
    xxxxx-1_PARAMETER_CHECK_HPP_ASSERT_REQUIRED_PARAMETER1(args, ignore, x5)
#define xxxxx-1_PARAMETER_CHECK_HPP_ASSERT_REQUIRED_PARAMETER6(args, ignore, x1, x2, x3, x4, x5, x6) \
    xxxxx-1_PARAMETER_CHECK_HPP_ASSERT_REQUIRED_PARAMETER5(args, ignore, x1, x2, x3, x4, x5);        \
    xxxxx-1_PARAMETER_CHECK_HPP_ASSERT_REQUIRED_PARAMETER1(args, ignore, x6)
#define xxxxx-1_PARAMETER_CHECK_HPP_ASSERT_REQUIRED_PARAMETER7(args, ignore, x1, x2, x3, x4, x5, x6, x7) \
    xxxxx-1_PARAMETER_CHECK_HPP_ASSERT_REQUIRED_PARAMETER6(args, ignore, x1, x2, x3, x4, x5, x6);        \
    xxxxx-1_PARAMETER_CHECK_HPP_ASSERT_REQUIRED_PARAMETER1(args, ignore, x7)
#define xxxxx-1_PARAMETER_CHECK_HPP_ASSERT_REQUIRED_PARAMETER8(args, ignore, x1, x2, x3, x4, x5, x6, x7, x8) \
    xxxxx-1_PARAMETER_CHECK_HPP_ASSERT_REQUIRED_PARAMETER7(args, ignore, x1, x2, x3, x4, x5, x6, x7);        \
    xxxxx-1_PARAMETER_CHECK_HPP_ASSERT_REQUIRED_PARAMETER1(args, ignore, x8)
#define xxxxx-1_PARAMETER_CHECK_HPP_ASSERT_REQUIRED_PARAMETER9(args, ignore, x1, x2, x3, x4, x5, x6, x7, x8, x9) \
    xxxxx-1_PARAMETER_CHECK_HPP_ASSERT_REQUIRED_PARAMETER8(args, ignore, x1, x2, x3, x4, x5, x6, x7, x8);        \
    xxxxx-1_PARAMETER_CHECK_HPP_ASSERT_REQUIRED_PARAMETER1(args, ignore, x9)
...
#define xxxxx-1_UNSUPPORTED_PARAMETER(args, parameter_type, whatfor)                                         \
    static_assert(                                                                                           \
        !xxxxx-1ing::internal::has_parameter_type<xxxxx-1ing::internal::ParameterType::parameter_type, args...>(), \
        "Parameter type " #parameter_type " is not supported " #whatfor "."                                  \
    )

/// @endcond

namespace xxxxx-1ing::internal {
/// @brief Struct wrapping a check that verifies that no unused parameters are part of the arguments.
///
/// @tparam RequiredParametersTuple All required xxxxx-1ing::internal::ParameterType passed as \c
/// std::integral_constant in an \c std::tuple.
/// @tparam OptionalParametersTuple All optional xxxxx-1ing::internal::ParameterType passed as \c
/// std::integral_constant in an \c std::tuple.
...
/// @brief Wrapper to get an \c std::integral_constant for a xxxxx-1ing::internal::ParameterType.
/// @tparam T xxxxx-1ing::internal::ParameterType that is converted to an \c std::integral_constant.
template <ParameterType T>
struct parameter_type_to_integral_constant {
    /// @brief xxxxx-1ing::internal::ParameterType as \c std::integral_constant.
    using type = std::integral_constant<ParameterType, T>;
};

/// @brief Wrapper to get a tuple of \c std::integral_constant for each xxxxx-1ing::internal::ParameterType
/// passed as template parameter that are extracted as tuple of \c std::integral_constant.
/// @tparam Parameters Passed xxxxx-1ing::internal::ParameterType.
...
/// @brief Wrapper to get a tuple of \c std::integral_constant for each xxxxx-1ing::internal::ParameterType
/// of the parameters.
/// @tparam Parameters Passed parameters for which the xxxxx-1ing::internal::ParameterType are extracted as
...
/// does not have to be computed/deduced by xxxxx-1).
///
...
/// @brief Checks if the buffer has to be computed by xxxxx-1ing, i.e. if it is an output parameter or the buffer has been
/// allocated by xxxxx-1.
...
/// @brief Checks if all buffers have to be computed by xxxxx-1ing, i.e., if all buffers are output parameters of the
/// buffers have been allocated by xxxxx-1ing.
...
/// @brief Checks if any of the buffers have to be computed by xxxxx-1ing, i.e., if at least one buffer is an output
/// parameter or has been allocated by xxxxx-1ing.
