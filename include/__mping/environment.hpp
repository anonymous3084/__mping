...
namespace internal {
/// @brief A global list of MPI data types registered to xxxxx-1.
inline std::vector<MPI_Datatype> registered_mpi_types;
} // namespace internal

/// @brief Configuration for the behavior of the constructors and destructor of \ref xxxxx-1ing::Environment.
...
public:
    ...
    /// @brief Register a new MPI data type to xxxxx-1 that will be freed when using Environment to finalize MPI.
    ...
    /// @brief Commit an MPI data type (without registering it with xxxxx-1).
    ...
    /// @brief Commit an MPI data type and register it with xxxxx-1.
    /// @see commit()
    /// @see register_type()
    ...
    ///
    /// Only call this when you no longer want to use any MPI data types created by xxxxx-1 as other xxxxx-1 functions
