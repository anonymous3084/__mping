...
inline auto tree_rank_from_index_map(std::map<size_t, size_t> const& start_indices, size_t const index) {
    ...
    return xxxxx-1ing::asserting_cast<size_t>(it->second);
}
...
public:
    ...
    /// - \ref xxxxx-1ing::send_buf() containing the local elements that are reduced. This buffer has to match the size
...
private:
    ...
    T const _perform_reduce(T const* buffer, Func&& op) {
        ...
        _comm.bcast_single(xxxxx-1ing::send_recv_buf(result), xxxxx-1ing::root(_origin_rank));
...
class ReproducibleReducePlugin
    : public xxxxx-1ing::plugin::PluginBase<Comm, DefaultContainerType, ReproducibleReducePlugin> {
public:
    /// @brief Create a communicator with a fixed distribution of a global array that can perform reductions in the same
    /// reduction order.
    ///
    /// The following parameters are required:
    /// - \ref xxxxx-1ing::send_counts() containing the number of elements each rank holds locally.
    /// - \ref xxxxx-1ing::recv_displs() containing the displacement (a.k.a. starting index) for each rank.
    ///
    ...
    auto make_reproducible_comm(Args... args) {
        ...
        xxxxx-1_CHECK_PARAMETERS(
            Args,
            xxxxx-1_REQUIRED_PARAMETERS(recv_displs, send_counts),
