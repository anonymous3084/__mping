...
public:
    ...
    auto recv(Args... args) const {
        xxxxx-1_CHECK_PARAMETERS(Args, xxxxx-1_REQUIRED_PARAMETERS(), xxxxx-1_OPTIONAL_PARAMETERS(recv_buf, recv_type));

        using default_recv_buf_type =
            decltype(xxxxx-1ing::recv_buf(alloc_new<
        ...
        auto   repack_recv_type = [&]() {
            // we cannot simply forward recv_type as xxxxx-1ing::recv_type as there are checks within recv() depending on
            // whether recv_type is caller provided or not
            if constexpr (internal::has_to_be_computed<decltype(recv_type)>) {
                return xxxxx-1ing::recv_type_out(recv_type.underlying());
            } else {
                return xxxxx-1ing::recv_type(recv_type.underlying());
            }
        };
        _comm.recv(
            xxxxx-1ing::recv_buf<recv_buf_type::resize_policy>(recv_buf.underlying()),
            repack_recv_type(),
            xxxxx-1ing::recv_count(recv_count_signed(recv_type.underlying())),
            xxxxx-1ing::source(_status.source_signed()),
            tag(_status.tag())
        );
...
struct PredicateForSparseAlltoall {
    ...
    static constexpr bool discard() {
        ...
        using ptypes_to_ignore = type_list<
            ...
            std::integral_constant<xxxxx-1ing::internal::ParameterType, xxxxx-1ing::internal::ParameterType::tag>,
            std::integral_constant<xxxxx-1ing::internal::ParameterType, xxxxx-1ing::internal::ParameterType::destination>>;
...
/// - \ref xxxxx-1ing::tag() the tag added to the directly exchanged messages. Defaults to the communicator's default tag
...
void SparseAlltoall<Comm, DefaultContainerType>::alltoallv_sparse(Args... args) const {
    ...
    using send_buf_param_type =
        std::integral_constant<sparse_alltoall::ParameterType, sparse_alltoall::ParameterType::sparse_send_buf>;
    auto const& dst_message_container = xxxxx-1ing::internal::select_parameter_type<send_buf_param_type>(args...);
    ...
    using message_value_type = typename xxxxx-1ing::internal::
        ValueTypeWrapper<xxxxx-1ing::internal::has_data_member_v<message_type>, message_type>::value_type;

    // Get tag
    using default_tag_buf_type = decltype(xxxxx-1ing::tag(self.default_tag()));
    auto const&& tag_param     = xxxxx-1ing::internal::select_parameter_type_or_default<
        xxxxx-1ing::internal::ParameterType::tag,
        default_tag_buf_type>(std::tuple(self.default_tag()), args...);

    //// Get callback
    using on_message_param_type =
        std::integral_constant<sparse_alltoall::ParameterType, sparse_alltoall::ParameterType::on_message>;
    auto const& on_message_cb = xxxxx-1ing::internal::select_parameter_type<on_message_param_type>(args...);

    RequestPool<DefaultContainerType> request_pool;
    for (auto const& [dst, msg]: dst_message_container.underlying()) {
        auto send_buf = xxxxx-1ing::send_buf(msg);
        ...
        if (send_buf.size() > 0) {
            ...
            auto      callable   = [&](auto... argsargs) {
                self.issend(
                    std::move(send_buf),
                    xxxxx-1ing::send_count(send_count),
                    destination(dst_),
