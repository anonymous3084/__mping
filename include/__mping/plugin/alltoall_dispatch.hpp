...
#include "xxxxx-1ing/plugin/alltoall_grid.hpp"
#include "xxxxx-1ing/plugin/alltoall_sparse.hpp"
...
struct PredicateDispatchAlltoall {
    ...
    static constexpr bool discard() {
        using ptypes_to_ignore = xxxxx-1ing::internal::type_list<
            std::integral_constant<
                dispatch_alltoall::ParameterType,
                dispatch_alltoall::ParameterType::comm_volume_threshold>,
            std::integral_constant<xxxxx-1ing::internal::ParameterType, xxxxx-1ing::internal::ParameterType::send_counts>>;
        using ptype_entry =
            std::integral_constant<xxxxx-1ing::internal::parameter_type_t<Arg>, xxxxx-1ing::internal::parameter_type_v<Arg>>;
...
public:
    ...
    auto alltoallv_dispatch(Args... args) const {
        ...
        size_t const max_bottleneck_send_volume =
            self.allreduce_single(xxxxx-1ing::send_buf(send_buf.size()), op(ops::max<size_t>{}));
        ...
        if (max_bottleneck_send_volume * sizeof(send_value_type) < volume_threshold.get_single_element()) {
            ...
            auto callable = [&](auto... argsargs) {
                initialize();
                return _grid_communicator.value().alltoallv(xxxxx-1ing::send_counts(send_counts), std::move(argsargs)...);
            };
        ...
        auto callable = [&](auto... argsargs) {
            return self.alltoallv(xxxxx-1ing::send_counts(send_counts), std::move(argsargs)...);
        };
        return std::apply(callable, filter_args());
    }
