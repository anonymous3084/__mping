...
public:
    using LevelCommunicator = xxxxx-1ing::Communicator<DefaultContainerType>; ///< Type of row and column communicator.
    ...
    template <
        ...
        typename... Plugins>
    GridCommunicator(xxxxx-1ing::Communicator<DefaultContainerType, Plugins...> const& comm)
    ...
    /// - \ref xxxxx-1ing::send_buf() containing the data that is sent to each rank. The size of this buffer has to be at
    ...
    /// - \ref xxxxx-1ing::send_displs() containing the number of elements to send to each rank.
    ///
    ...
    auto alltoallv_with_envelope(Args... args) const {
        xxxxx-1_CHECK_PARAMETERS(
            ...
            xxxxx-1_OPTIONAL_PARAMETERS(send_displs)
        );
        // Get send_buf
        auto const& send_buf =
    ...
    ///
    /// - \ref xxxxx-1ing::recv_buf() containing a buffer for the output. Afterwards, this buffer will contain
    ///
    /// the data received as specified for send_buf. The buffer will be resized according to the buffer's
    /// xxxxx-1ing::BufferResizePolicy. If resize policy is xxxxx-1ing::BufferResizePolicy::no_resize, the buffer's underlying
    ...
    auto alltoallv(Args... args) const {
        xxxxx-1_CHECK_PARAMETERS(
            Args,
            xxxxx-1_REQUIRED_PARAMETERS(send_buf, send_counts),
            xxxxx-1_OPTIONAL_PARAMETERS(send_displs, recv_buf, recv_counts, recv_displs)
        );
        ...
        auto grid_recv_buf = alltoallv_with_envelope<envelope_level>(
            std::move(send_buf),
            xxxxx-1ing::send_counts(send_counts.underlying()),
            xxxxx-1ing::send_displs(send_displs.underlying())
        );
...
private:
    ...
    rowwise_exchange(SendBuffer const& send_buf, SendCounts const& send_counts, SendDispls const& send_displs) const {
        ...
        return _row_comm.alltoallv(
            xxxxx-1ing::send_buf(rowwise_send_buf),
            xxxxx-1ing::send_counts(row_send_counts),
            xxxxx-1ing::send_displs(row_send_displs)
        );
    }
    ...
    auto columnwise_exchange(RowwiseRecvBuf<grid_plugin_helpers::MessageEnvelope<EnvelopeArgs...>>&& rowwise_recv_buf
        ...
        return _column_comm.alltoallv(
            xxxxx-1ing::send_buf(colwise_send_buf),
            xxxxx-1ing::send_counts(send_counts),
            xxxxx-1ing::send_displs(send_displacements)
        );
    }
...
private:
    ...
    size_t                                      _num_columns;
    xxxxx-1ing::Communicator<DefaultContainerType> _row_comm;
    xxxxx-1ing::Communicator<DefaultContainerType> _column_comm;
};
} // namespace grid
...
public:
    /// @brief Returns a \ref xxxxx-1ing::plugin::GridCommunicator.
