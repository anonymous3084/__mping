...
//
// Copyright 2023-2024 The xxxxx-1 Authors
//
...
///
/// In contrast to \ref xxxxx-1ing::Communicator::recv(), this method does not block if no message is available. Instead,
...
/// - \ref xxxxx-1ing::recv_buf() the buffer to receive the message into. The buffer's underlying storage must be large
/// enough to hold all received elements. If no \ref xxxxx-1ing::recv_buf() is provided, the \c value_type of the recv
/// buffer has to be passed as a template parameter to \c recv().
///
/// - \ref xxxxx-1ing::tag() receive message with the given tag. Defaults to receiving for an arbitrary tag, i.e. \c
/// tag(tags::any).
///
/// - \ref xxxxx-1ing::source() receive a message sent from the given source rank. Defaults to probing for an arbitrary
/// source, i.e. \c source(rank::any).
///
/// - \ref xxxxx-1ing::status(). Returns info about the received message by setting the
/// appropriate fields in the status object. The status can be obtained by using \c xxxxx-1ing::status_out and ignored by
/// passing \c xxxxx-1ing::ignore<>. This is the default.
///
/// - \ref xxxxx-1ing::recv_type() specifying the \c MPI datatype to use as the recv type. If omitted, the \c MPI datatype
/// is derived automatically based on `recv_buf`'s underlying \c value_type.
///
/// @tparam recv_value_type_tparam The type that is received. Only required when no \ref xxxxx-1ing::recv_buf() is given.
...
/// xxxxx-1ing::MPIResult. If the result object is empty, i.e. there are no owning out parameters passed to `try_recv` (see
...
auto xxxxx-1ing::Communicator<DefaultContainerType, Plugins...>::try_recv(Args... args) const {
    // Check parameters
    xxxxx-1_CHECK_PARAMETERS(
        Args,
        xxxxx-1_REQUIRED_PARAMETERS(),
        xxxxx-1_OPTIONAL_PARAMETERS(recv_buf, tag, source, status, recv_type)
    );
