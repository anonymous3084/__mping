...
/// @brief Deduce the MPI_Datatype to use as send_type in a p2p send operation.If \ref xxxxx-1ing::send_type() is given,
...
constexpr auto determine_mpi_send_datatype(Args&... args)
    -> decltype(internal::select_parameter_type_or_default<
                    ...
                    decltype(xxxxx-1ing::send_type_out())>(std::make_tuple(), args...)
...
/// @brief Deduce the MPI_Datatype to use as recv_type in a p2p recv operation.If \ref xxxxx-1ing::recv_type() is given,
...
constexpr auto determine_mpi_recv_datatype(Args&... args)
    -> decltype(internal::select_parameter_type_or_default<
                    ...
                    decltype(xxxxx-1ing::recv_type_out())>(std::make_tuple(), args...)
                    .construct_buffer_or_rebind()) {
    // Get the recv type
    ...
    static_assert(
        ...
        "If a custom recv type is given, xxxxx-1ing is not able to deduce the correct size of the "
        "recv buffer. "
