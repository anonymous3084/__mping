...
#include "xxxxx-1ing/measurements/measurement_aggregation_definitions.hpp"

namespace xxxxx-1ing::measurements::internal {
...
/// xxxxx-1erTreeNode via CRTP paradigm.
...
///
/// @tparam xxxxx-1ePoint Type of a point in time.
/// @tparam Duration  Type of a duration.
template <typename xxxxx-1ePoint, typename Duration>
class xxxxx-1erTreeNode : public TreeNode<xxxxx-1erTreeNode<xxxxx-1ePoint, Duration>> {
public:
    using TreeNode<xxxxx-1erTreeNode<xxxxx-1ePoint, Duration>>::TreeNode;
    ...
    xxxxx-1ePoint startpoint() const {
        return _start;
    }
    ...
    void startpoint(xxxxx-1ePoint start) {
        _start = start;
    }
    ...
    /// @param mode The xxxxx-1ing::measurements::KeyAggregationMode parameter determines how multiple time measurements
...
private:
    xxxxx-1ePoint                          _start; ///< Point in time at which the current measurement has been started.
...
/// @brief Tree consisting of objects of type xxxxx-1erTreeNode. The tree constitutes a hierarchy of time measurements
...
template <typename xxxxx-1ePoint, typename Duration>
struct xxxxx-1erTree {
    /// @brief Construct a xxxxx-1erTree consisting only of a root node.
    xxxxx-1erTree() : root{"root"}, current_node(&root) {
        root.parent_ptr() = &root;
    }

    /// @brief Resets the root node (i.e. deletes and assigns a new empty node).
    void reset() {
        root         = xxxxx-1erTreeNode<xxxxx-1ePoint, Duration>{"root"};
        current_node = &root;
    }

    xxxxx-1erTreeNode<xxxxx-1ePoint, Duration>  root;         ///< Root node of the tree.
    xxxxx-1erTreeNode<xxxxx-1ePoint, Duration>* current_node; ///< Pointer to the currently active node of the tree.
};
