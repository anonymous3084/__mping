...
#include "xxxxx-1ing/measurements/aggregated_tree_node.hpp"
#include "xxxxx-1ing/measurements/internal/measurement_utils.hpp"

namespace xxxxx-1ing::measurements {

/// @brief Distributed timer object.
///
/// ## xxxxx-1er hierarchy: ##
/// The timer is able to execute time measurements in a hierarchical manner.
/// xxxxx-1e measurements are executed with a matching pair of calls to xxxxx-1er::start() and xxxxx-1er::stop().
/// Each call to start() enters a new level in the hierarchy which is left again with the corresponding call to
/// stop(). xxxxx-1e measurements can be nested and the parent time measurement remains active while its child time
...
///
///
/// @see The usage example for xxxxx-1er provides some more information on how the class can be used.
///
...
public:
    ...
    /// @todo Add a toggle option to the xxxxx-1er class to set the default key aggregation behaviour for stop().
    ...
    /// xxxxx-1erTreeNode::data_aggregation_operations().
...
private:
    internal::xxxxx-1erTree<double, Duration>
                            _timer_tree; ///< xxxxx-1er tree used to represent the hierarchical time measurements.
    ...
    /// @brief Traverses and evaluates the given xxxxx-1erTreeNode and stores the result in the corresponding
    /// AggregatedTreeNode
    ///
    ...
    void aggregate(
        AggregatedTreeNode<Duration>& evaluation_tree_node, internal::xxxxx-1erTreeNode<double, Duration>& timer_tree_node
    ) {
        KASSERT(
            internal::is_string_same_on_all_ranks(timer_tree_node.name(), _comm),
            "Currently processed xxxxx-1erTreeNode has not the same name on all ranks -> timers have diverged",
            assert::heavy_communication
        );
        KASSERT(
            _comm.is_same_on_all_ranks(timer_tree_node.durations().size()),
            "Currently processed xxxxx-1erTreeNode has not the same number of measurements on all ranks -> timers have "
            "diverged",
    ...
    void aggregate_measurements_globally(
        ...
        xxxxx-1ing::measurements::AggregatedTreeNode<double>& evaluation_node
    ) {
        switch (mode) {
...
/// @brief Gets a reference to a xxxxx-1ing::measurements::Basicxxxxx-1er.
///
/// @return A reference to a xxxxx-1ing::measurements::BasicCommunicator.
inline xxxxx-1er<Communicator<>>& timer() {
    static xxxxx-1er<Communicator<>> timer;
    return timer;
}
