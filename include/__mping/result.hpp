...
/// @file
/// @brief Some functions and types simplifying/enabling the development of wrapped \c MPI calls in xxxxx-1.
...
xxxxx-1_MAKE_HAS_MEMBER(extract)
...
/// @brief MPIResult contains the result of a \c MPI call wrapped by xxxxx-1.
///
...
/// by/transferred to xxxxx-1, the content of the buffers can be extracted using
/// extract_<result>.
...
public:
    ...
    /// @brief Get the \c xxxxx-1ing::Status from the MPIResult object.
    ///
    ...
    /// @brief Extracts the \c xxxxx-1ing::Status from the MPIResult object.
    ///
    /// This function is only available if the underlying status is owned by the
    /// MPIResult object.
    /// @tparam T Template parameter helper only needed to remove this function if the corresponding data is not part of
...
/// @brief Specialization of the std::tuple_size for \ref xxxxx-1ing::MPIResult. Part of the structured binding machinery.
///
/// @tparam Args Automatically deducted template parameters.
template <typename... Args>
struct tuple_size<xxxxx-1ing::MPIResult<Args...>> {
    static constexpr size_t value = sizeof...(Args); ///< Number of data buffers in the \ref xxxxx-1ing::MPIResult.
};

/// @brief Specialization of the std::tuple_element for \ref xxxxx-1ing::MPIResult. Part of the structured binding
/// machinery.
///
/// @param index Index of the entry of \ref xxxxx-1ing::MPIResult for which the underlying data type shall be deduced.
/// @tparam Args Automatically deducted template parameters.
template <size_t index, typename... Args>
struct tuple_element<index, xxxxx-1ing::MPIResult<Args...>> {
    using type = std::remove_reference_t<decltype(declval<xxxxx-1ing::MPIResult<Args...>>().template get<index>()
...
/// @brief NonBlockingResult contains the result of a non-blocking \c MPI call wrapped by xxxxx-1. It encapsulates a
/// \ref xxxxx-1ing::MPIResult and a \ref xxxxx-1ing::Request.
///
///
...
public:
    ...
    /// @param result The underlying \ref xxxxx-1ing::MPIResult.
    /// @param request A \ref xxxxx-1ing::internal::DataBuffer containing the associated \ref xxxxx-1ing::Request.
    NonBlockingResult(std::unique_ptr<MPIResultType> result, RequestDataBuffer request)
        : _mpi_result(std::move(result)),
          _request(std::move(request)) {}

    /// @brief \c true if the result object owns the underlying \ref xxxxx-1ing::Request.
    ...
    ///
    /// If \p is \c xxxxx-1ing::status(ignore<>), or not an out-paramter:
    ...
    ///
    /// @param status A parameter created by \ref xxxxx-1ing::status() or \ref xxxxx-1ing::status_out().
    /// Defaults to \c xxxxx-1ing::status(ignore<>).
    template <
    ...
    auto wait(StatusParamObjectType status = xxxxx-1ing::status(ignore<>)) {
        static_assert(
    ...
    template <
        ...
        typename NonBlockingResulType_ = NonBlockingResult<MPIResultType, RequestDataBuffer>,
        typename std::enable_if<NonBlockingResulType_::owns_request, bool>::type = true>
    auto test(StatusParamObjectType status = xxxxx-1ing::status(ignore<>)) {
        static_assert(
            StatusParamObjectType::parameter_type == internal::ParameterType::status,
            "Only status parameters are allowed."
        );
...
/// In this case a \ref xxxxx-1ing::MPIResult object is created, which stores the buffers to return (owning out buffers)
...
/// In this case a \ref xxxxx-1ing::MPIResult object is created, which stores the buffers to return. The
...
/// \ref xxxxx-1ing::MPIResult object stores the buffer to return (owning buffers for which a *_out() named parameter was
...
/// @brief Factory for creating a \ref xxxxx-1ing::NonBlockingResult.
///
/// Makes an \ref xxxxx-1ing::NonBlockingResult from all arguments passed and inserts internal::ResultCategoryNotUsed
/// when no fitting parameter type is passed as argument.
///
/// Note that an argument of with type \ref xxxxx-1ing::internal::ParameterType::request is required.
///
/// @tparam CallerProvidedArgs Types of arguments passed to the wrapped MPI call.
/// @tparam Args Automatically deducted template parameters.
/// @param args All parameter that should be included in the MPIResult.
/// @return \ref xxxxx-1ing::NonBlockingResult encapsulating all passed parameters.
