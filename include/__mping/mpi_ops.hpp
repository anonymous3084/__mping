...
/// @brief Template specialization for \c xxxxx-1ing::internal::max_impl without type parameter, which leaves the operand
/// type to be deduced.
///
/// The actual implementation is used in case that the operation is a builtin operation for the given datatype.
template <>
...
/// @brief Template specialization for \c xxxxx-1ing::internal::min_impl without type parameter, which leaves the operand
/// type to be deduced.
/// The actual implementation is used in case that the operation is a builtin operation for the given datatype.
template <>
...
/// @brief Template specialization for \c xxxxx-1ing::internal::logical_xor_impl without type parameter, which leaves to
...
/// the aliases \c xxxxx-1ing::ops::plus, \c xxxxx-1ing::ops::multiplies, \c xxxxx-1ing::ops::max(), ...
...
using max = xxxxx-1ing::internal::max_impl<T>;

/// @brief builtin minimum operation (aka `MPI_MIN`)
template <typename T = void>
using min = xxxxx-1ing::internal::min_impl<T>;
...
template <typename T = void>
using logical_xor = xxxxx-1ing::internal::logical_xor_impl<T>;
...
#ifdef xxxxx-1_DOXYGEN_ONLY
/// @brief Type trait for checking whether a functor is a builtin MPI reduction operation and query corresponding \c
/// MPI_Op.
///
/// Example:
/// @code
/// is_builtin_mpi_op<xxxxx-1ing::ops::plus<>, int>::value // true
/// is_builtin_mpi_op<xxxxx-1ing::ops::plus<>, int>::op()  // MPI_SUM
...
struct mpi_operation_traits {
    ...
    /// defined in \c xxxxx-1ing::ops and there corresponding type-aliased equivalents in the standard library.
...
/// @brief Helper function that maps an \c MPI_Op to the matching functor from \c xxxxx-1ing::ops. In case no function
/// maps, the functor is called with \c xxxxx-1ing::ops::null<>{}.
///
/// @param op The operation.
/// @param func The lambda to be called with the functor matching the \c MPI_Op, e.g. in the case of \c MPI_SUM we call
/// \c func(xxxxx-1ing::ops::plus<>{}).
...
class ReduceOperation {
    static_assert(
        std::is_same_v<
            Commutative,
            xxxxx-1ing::ops::internal::
                commutative_tag> || std::is_same_v<Commutative, xxxxx-1ing::ops::internal::non_commutative_tag>,
...
public:
    ...
    static constexpr bool is_builtin  = false;
    static constexpr bool commutative = std::is_same_v<Commutative, xxxxx-1ing::ops::internal::commutative_tag>;
...
class ReduceOperation<T, Op, Commutative, typename std::enable_if<mpi_operation_traits<Op, T>::is_builtin>::type> {
    static_assert(
        std::is_same_v<Commutative, xxxxx-1ing::ops::internal::undefined_commutative_tag>,
