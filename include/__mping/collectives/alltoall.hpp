...
#include "xxxxx-1ing/collectives/ibarrier.hpp"
...
#include "xxxxx-1ing/p2p/iprobe.hpp"
...
/// This parameter is mandatory if \ref xxxxx-1ing::send_type() is given.
///
...
///
/// - \ref xxxxx-1ing::recv_buf() specifying a buffer for the output. A buffer of at least
...
auto xxxxx-1ing::Communicator<DefaultContainerType, Plugins...>::alltoall(Args... args) const {
    ...
    if constexpr (inplace) {
        ...
        xxxxx-1_CHECK_PARAMETERS(
            ...
            xxxxx-1_OPTIONAL_PARAMETERS(recv_buf, send_count, recv_count, send_type, recv_type)
        );
...
///
/// - \ref xxxxx-1ing::send_recv_buf() containing the data that is sent to each rank and received from each rank. The size
...
/// The following parameters are optional:
///
/// - \ref xxxxx-1ing::send_recv_count() specifying how many elements are sent and received. If
/// omitted, the size of send_recv_buf divided by communicator size is used.
/// This parameter is mandatory if \ref xxxxx-1ing::send_recv_type() is given.
/// -
...
template <
    ...
    typename... Plugins>
template <typename... Args>
auto xxxxx-1ing::Communicator<DefaultContainerType, Plugins...>::alltoall_inplace(Args... args) const {
    using namespace internal;
    xxxxx-1_CHECK_PARAMETERS(
        Args,
    ...
    KASSERT(
        // if the type is user provided, xxxxx-1ing cannot make any assumptions about the required size of the
        // buffer
...
/// buffers are required:
/// - \ref xxxxx-1ing::send_buf() containing the data that is sent to each rank. The size of this buffer has to be at least
/// the sum of the send_counts argument.
///
/// - \ref xxxxx-1ing::send_counts() containing the number of elements to send to each rank.
///
/// The following parameters are optional but result in communication overhead if omitted:
/// - \ref xxxxx-1ing::recv_counts() containing the number of elements to receive from each rank.
/// This parameter is mandatory if \ref xxxxx-1ing::recv_type() is given.
///
...
///
/// - \ref xxxxx-1ing::send_displs() containing the offsets of the messages in send_buf. The `send_counts[i]` elements
...
template <
    ...
    typename... Plugins>
template <typename... Args>
auto xxxxx-1ing::Communicator<DefaultContainerType, Plugins...>::alltoallv(Args... args) const {
    // Get all parameter objects
    xxxxx-1_CHECK_PARAMETERS(
        Args,
        xxxxx-1_REQUIRED_PARAMETERS(send_buf, send_counts),
        xxxxx-1_OPTIONAL_PARAMETERS(recv_counts, recv_buf, send_displs, recv_displs, send_type, recv_type)
    );
    ...
    // Get send_displs
    using default_send_displs_type = decltype(xxxxx-1ing::send_displs_out(alloc_new<DefaultContainerType<int>>));
    auto&& send_displs =
    ...
    if constexpr (do_calculate_recv_counts) {
        ...
        this->alltoall(xxxxx-1ing::send_buf(send_counts.get()), xxxxx-1ing::recv_buf(recv_counts.get()));
    } else {
        KASSERT(recv_counts.size() >= this->size(), "Recv counts buffer is not large enough.", assert::light);
    }
    ...
    KASSERT(
        // if the send type is user provided, xxxxx-1ing cannot make any assumptions about the size of the send
        // buffer
