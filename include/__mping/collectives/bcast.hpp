...
/// - \ref xxxxx-1ing::send_recv_buf() containing the data that is sent to the other ranks. Non-root ranks must allocate
...
/// xxxxx-1ing::BufferResizePolicy.
///
/// The following parameter is optional but causes additional communication if not present.
/// - \ref xxxxx-1ing::send_recv_count() specifying how many elements are broadcasted. This parameter must be given either
/// on all or none of the ranks. If not specified, the count is set to the size of xxxxx-1ing::send_recv_buf() on
/// root and broadcasted to all other ranks. This parameter is mandatory if \ref xxxxx-1ing::send_recv_type() is given.
///
/// The following parameter are optional:
/// - \ref xxxxx-1ing::send_recv_type() specifying the \c MPI datatype to use as send type on the root PE and recv type on
/// all non-root PEs. If omitted, the \c MPI datatype is derived automatically based on send_recv_buf's underlying \c
/// value_type.
///
/// - \ref xxxxx-1ing::root() specifying an alternative root. If not present, the default root of the \c
/// Communicator is used, see root().
///
/// @tparam recv_value_type_tparam The type that is received. Only required when no \ref xxxxx-1ing::send_recv_buf() is
/// given.
...
auto xxxxx-1ing::Communicator<DefaultContainerType, Plugins...>::bcast(Args... args) const {
    using namespace ::xxxxx-1ing::internal;
    xxxxx-1_CHECK_PARAMETERS(
        Args,
        xxxxx-1_REQUIRED_PARAMETERS(),
        xxxxx-1_OPTIONAL_PARAMETERS(send_recv_buf, root, send_recv_count, send_recv_type)
    );
    ...
    using default_send_recv_buf_type =
        decltype(xxxxx-1ing::send_recv_buf(alloc_new<DefaultContainerType<recv_value_type_tparam>>));
    auto&& send_recv_buf =
    ...
    if constexpr (is_serialization_used) {
        xxxxx-1_UNSUPPORTED_PARAMETER(Args, send_recv_count, when using serialization);
        xxxxx-1_UNSUPPORTED_PARAMETER(Args, send_recv_type, when using serialization);
...
///
/// The following buffer is required on the root rank:
/// - \ref xxxxx-1ing::send_recv_buf() containing the single value that is sent to the other ranks. Non-root ranks must
...
/// - \ref xxxxx-1ing::root() specifying an alternative root. If not present, the default root of the \c Communicator is
/// used, see root().
///
...
template <
    ...
    typename... Plugins>
template <typename recv_value_type_tparam /* = xxxxx-1ing::internal::unused_tparam */, typename... Args>
auto xxxxx-1ing::Communicator<DefaultContainerType, Plugins...>::bcast_single(Args... args) const {
    ...
    xxxxx-1_CHECK_PARAMETERS(Args, xxxxx-1_REQUIRED_PARAMETERS(), xxxxx-1_OPTIONAL_PARAMETERS(send_recv_buf, root));
