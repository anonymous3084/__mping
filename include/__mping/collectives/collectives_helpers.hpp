...
size_t compute_required_recv_buf_size_in_vectorized_communication(
    ...
    if constexpr (do_calculate_recv_displs) {
        ...
        // xxxxx-1ing to resize the receive buffer.
        int recv_buf_size = 0;
...
/// If \ref xxxxx-1ing::send_type() is given, the \c MPI_Datatype wrapped inside will be used as send_type. Otherwise, the
/// \c MPI_datatype is derived automatically based on send_buf's underlying \c value_type.
///
/// If \ref xxxxx-1ing::recv_type()
...
constexpr auto determine_mpi_datatypes(Args&... args) {
    ...
    static_assert(
        ...
        "If a custom recv type is given, xxxxx-1ing is not able to deduce the correct size of the recv buffer. "
    ...
    using default_mpi_send_type = decltype(xxxxx-1ing::send_type_out());
    using default_mpi_recv_type = decltype(xxxxx-1ing::recv_type_out());
...
/// of MPI_Datatype instead of (possibly) distinct send and recv types. If \ref xxxxx-1ing::send_recv_type() is given, the
...
constexpr auto determine_mpi_send_recv_datatype(Args&... args)
    -> decltype(internal::select_parameter_type_or_default<
                    ...
                    decltype(xxxxx-1ing::send_recv_type_out())>(std::make_tuple(), args...)
                    .construct_buffer_or_rebind()) {
    // Some assertions:
    ...
    static_assert(
        ...
        "If a custom send_recv type is given, xxxxx-1ing is not able to deduce the correct size of the "
    ...
    using default_mpi_send_recv_type = decltype(xxxxx-1ing::send_recv_type_out());
