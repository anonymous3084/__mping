...
/// - \ref xxxxx-1ing::send_buf() containing the data that is sent to the root. This buffer has to be the same size at
/// each rank. See allgather_v if the amounts differ.
///
/// The following parameters are optional:
/// - \ref xxxxx-1ing::send_count() specifying how many elements are sent. If
/// omitted, the size of the send buffer is used. This parameter is mandatory if \ref xxxxx-1ing::send_type() is given.
///
...
///
/// - \ref xxxxx-1ing::recv_count() specifying how many elements are received. If
/// omitted, the value of send_counts will be used. This parameter is mandatory if \ref xxxxx-1ing::recv_type() is given.
///
/// - \ref xxxxx-1ing::recv_buf() specifying a buffer for the output. Afterwards, this buffer will contain
...
auto xxxxx-1ing::Communicator<DefaultContainerType, Plugins...>::allgather(Args... args) const {
    ...
    if constexpr (inplace) {
        ...
        xxxxx-1_CHECK_PARAMETERS(
            ...
            xxxxx-1_OPTIONAL_PARAMETERS(send_count, recv_count, recv_buf, send_type, recv_type)
        );
        ...
        KASSERT(
            // if the send type is user provided, xxxxx-1ing no longer can deduce the number of elements to send from the
...
///
/// The following parameters are required:
/// - \ref xxxxx-1ing::send_recv_buf() containing the data that is sent to the root. Opposed to the non-inplace version,
...
///
/// The following parameters are optional:
/// - \ref xxxxx-1ing::send_recv_count() specifying how many elements are sent and received. If omitted, the size
/// `send_recv_buf.size() / size()` is used.
///
/// - \ref xxxxx-1ing::send_recv_type() specifying the \c MPI datatype to use as send and recv type. If omitted, the \c MPI
...
template <
    ...
    typename... Plugins>
template <typename... Args>
auto xxxxx-1ing::Communicator<DefaultContainerType, Plugins...>::allgather_inplace(Args... args) const {
    using namespace xxxxx-1ing::internal;
    xxxxx-1_CHECK_PARAMETERS(
        Args,
        xxxxx-1_REQUIRED_PARAMETERS(send_recv_buf),
    ...
    KASSERT(
        // if the type is user provided, xxxxx-1ing no longer can deduce the number of elements to send from the size
        // of the recv buffer
    ...
    KASSERT(
        // if the type is user provided, xxxxx-1ing cannot make any assumptions about the required size of the buffer
...
///
/// The following parameters are required:
/// - xxxxx-1ing::send_buf() containing the data that is sent to all other ranks.
///
/// The following parameters are optional but result in communication overhead if omitted:
/// - \ref xxxxx-1ing::recv_counts() containing the number of elements to receive from each rank. This parameter is
/// mandatory if \ref xxxxx-1ing::recv_type() is given.
///
/// The following parameters are optional:
/// - xxxxx-1ing::send_count() specifying how many elements are sent. If
/// omitted, the size of the send buffer is used. This parameter is mandatory if \ref xxxxx-1ing::send_type() is given.
///
/// - \ref xxxxx-1ing::send_type() specifying the \c MPI datatype to use as send type. If omitted, the \c
/// MPI datatype is derived automatically based on send_buf's underlying \c value_type.
///
/// - xxxxx-1ing::recv_buf() specifying a buffer for the output.  Afterwards, this buffer will contain
...
///
/// - xxxxx-1ing::recv_displs() containing the offsets of the messages in recv_buf. The `recv_counts[i]`
...
///
/// - \ref xxxxx-1ing::recv_type() specifying the \c MPI datatype to use as recv type. If omitted, the \c
...
template <
    ...
    typename... Plugins>
template <typename... Args>
auto xxxxx-1ing::Communicator<DefaultContainerType, Plugins...>::allgatherv(Args... args) const {
    using namespace xxxxx-1ing::internal;
    xxxxx-1_CHECK_PARAMETERS(
        Args,
        xxxxx-1_REQUIRED_PARAMETERS(send_buf),
        xxxxx-1_OPTIONAL_PARAMETERS(send_count, recv_buf, recv_counts, recv_displs, send_type, recv_type)
    );
    ...
    if constexpr (do_calculate_recv_counts) {
        ...
        this->allgather(
            xxxxx-1ing::send_buf(static_cast<int>(send_count.get_single_element())),
    ...
    recv_buf.resize_if_requested(compute_required_recv_buf_size);
    KASSERT(
        // if the recv type is user provided, xxxxx-1ing cannot make any assumptions about the required size of the recv
        // buffer
        recv_type_is_input_parameter || recv_buf.size() >= compute_required_recv_buf_size(),
        "Recv buffer is not large enough to hold all received elements.",
        assert::light
