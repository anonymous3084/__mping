...
/// not intended to be used directly. Instead, use \ref xxxxx-1ing::Request or
/// \ref xxxxx-1ing::PooledRequest or define your own request type, which must
...
public:
    ...
    /// MPI_REQUEST_NULL.
    ///
    /// @param status_param A parameter created by \ref xxxxx-1ing::status() or \ref xxxxx-1ing::status_out().
    /// Defaults to \c xxxxx-1ing::status(ignore<>).
    ///
    /// @return The status object, if \p status is \ref xxxxx-1ing::status_out(), otherwise nothing.
    template <typename StatusParamObjectType = decltype(status(ignore<>))>
    auto wait(StatusParamObjectType status_param = xxxxx-1ing::status(ignore<>)) {
        static_assert(
    ...
    /// Defaults to \c xxxxx-1ing::status(ignore<>).
    ///
    /// @return Returns \c true if the underlying request is complete. If \p status is \ref xxxxx-1ing::status_out() and
    /// owning, returns an \c std::optional encapsulating the status in case of completion, \c std::nullopt otherwise.
    template <typename StatusParamObjectType = decltype(status(ignore<>))>
    [[nodiscard]] auto test(StatusParamObjectType status_param = xxxxx-1ing::status(ignore<>)) {
        static_assert(
            StatusParamObjectType::parameter_type == internal::ParameterType::status,
            "Only status parameters are allowed."
        );
...
/// @param requests A (contiguous) container of \ref xxxxx-1ing::Request.
/// @tparam Container The container type.
template <
    typename Container,
    typename std::enable_if<
...
/// This incurs overhead for copying the request handles to an intermediate container.
/// @param args A list of request handles to wait on. These may be lvalues or rvalues convertible \ref xxxxx-1ing::Request.
template <
