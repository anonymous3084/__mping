...
TEST_F(EnvironmentTest, wtime) {
    ...
    Environment<xxxxx-1ing::InitMPIMode::NoInitFinalize> env;
...
TEST_F(EnvironmentTest, wtick) {
    Environment<xxxxx-1ing::InitMPIMode::NoInitFinalize> env;
    double                                            xxxxx-1ing_wtick = env.wtick();
    EXPECT_DOUBLE_EQ(xxxxx-1ing_wtick, MPI_Wtick());

    xxxxx-1ing_wtick = Environment<>::wtick();
    EXPECT_DOUBLE_EQ(xxxxx-1ing_wtick, MPI_Wtick());
}
...
TEST_F(EnvironmentTest, buffer_attach_and_detach) {
    ...
    buffer.resize(42);
    env.buffer_attach(xxxxx-1ing::Span<int>{buffer.begin(), buffer.end()});
...
TEST_F(EnvironmentTest, buffer_attach_and_detach_with_other_type) {
    ...
    env.buffer_attach(xxxxx-1ing::Span<attach_type>{buffer.begin(), buffer.end()});
...
TEST_F(EnvironmentTest, buffer_attach_multiple_fails) {
    ...
    env.buffer_attach(xxxxx-1ing::Span<int>{buffer1.begin(), buffer1.end()});
    EXPECT_EQ(attached_buffer_ptr, buffer1.data());
    EXPECT_EQ(attached_buffer_size, 2 * Environment<>::bsend_overhead * sizeof(int));

#if KASSERT_ENABLED(xxxxx-1_ASSERTION_LEVEL_NORMAL)
    EXPECT_KASSERT_FAILS(
        env.buffer_attach(xxxxx-1ing::Span<int>{buffer2.begin(), buffer2.end()}),
