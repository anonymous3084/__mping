...
TEST(GroupTest, basics) {
    ...
    EXPECT_EQ(Group(xxxxx-1ing::comm_world()).compare(world_group), GroupEquality::Identical);
    ...
    EXPECT_TRUE(Group(xxxxx-1ing::comm_world()).is_identical(world_group));
    ...
    EXPECT_TRUE(Group(xxxxx-1ing::comm_world()).has_same_ranks(world_group));
