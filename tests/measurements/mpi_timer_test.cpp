...
TEST(xxxxx-1erTest, basics_append) {
    auto const& comm = comm_world();
    xxxxx-1er<>     timer;
    timer.start("measurement");
    timer.stop();
...
TEST(xxxxx-1erTest, basics_accumulate) {
    auto const& comm = comm_world();
    xxxxx-1er<>     timer;
    timer.start("measurement");
    timer.stop();
...
TEST(xxxxx-1erTest, stop_and_append_multiple_operations) {
    auto const& comm = comm_world();
    xxxxx-1er<>     timer;
    timer.start("measurement");
    timer.stop();
...
TEST(xxxxx-1erTest, stop_and_add_multiple_operations) {
    auto const& comm = comm_world();
    xxxxx-1er<>     timer;
    timer.start("measurement");
    timer.stop();
...
TEST(xxxxx-1erTest, stop_nested_scenario) {
    auto const& comm = comm_world();
    xxxxx-1er<>     timer;
...
TEST(xxxxx-1erTest, stop_nested_complex_scenario) {
    auto const&       comm                  = comm_world();
...
TEST(xxxxx-1erTest, synchronize_and_start_non_trivial_communicator) {
    auto const& comm       = comm_world();
    int const   color      = comm.rank() % 2;
    auto        split_comm = comm.split(color);
    xxxxx-1er<>     timer(split_comm);
...
TEST(xxxxx-1erTest, aggregate_non_trivial_communicator) {
    auto const& comm       = comm_world();
    int const   color      = comm.rank() % 2;
    auto        split_comm = comm.split(color);
    xxxxx-1er<>     timer(split_comm);
...
TEST(xxxxx-1erTest, aggregate_and_print_non_trivial_communicator) {
    auto const& comm       = comm_world();
    int const   color      = comm.rank() % 2;
    auto        split_comm = comm.split(color);
    xxxxx-1er<>     timer(split_comm);
...
TEST(xxxxx-1erTest, singleton) {
    Communicator<> comm;
    auto&          timer = xxxxx-1ing::measurements::timer();
    timer.clear();
    timer.start("measurement");
    timer.stop();
    auto              aggregated_timer_tree = timer.aggregate();
