...
#include "xxxxx-1ing/plugin/reproducible_reduce.hpp"

using namespace xxxxx-1ing::plugin::reproducible_reduce;
...
std::vector<T> scatter_array(C comm, std::vector<T> const& global_array, Distribution const d) {
    ...
    comm.scatterv(
        xxxxx-1ing::send_buf(global_array),
        xxxxx-1ing::recv_buf<xxxxx-1ing::BufferResizePolicy::resize_to_fit>(result),
        xxxxx-1ing::send_counts(d.send_counts),
        xxxxx-1ing::send_displs(d.displs)
    );
...
auto distribute_evenly(size_t const collection_size, size_t const comm_size) {
    ...
    std::vector<int> send_counts(comm_size, xxxxx-1ing::asserting_cast<int>(elements_per_rank));
...
auto distribute_randomly(size_t const collection_size, size_t const comm_size, size_t const seed) {
    ...
    std::uniform_int_distribution<> dist(0, xxxxx-1ing::asserting_cast<int>(collection_size));

    // See https://stackoverflow.com/a/48205426 for details
    std::vector<int> points(comm_size, 0UL);
    points.push_back(xxxxx-1ing::asserting_cast<int>(collection_size));
...
TEST(ReproducibleReduceTest, Log2l) {
    using xxxxx-1ing::plugin::reproducible_reduce::log2l;
...
#if KASSERT_ENABLED(xxxxx-1_ASSERTION_LEVEL_NORMAL)
...
TEST(ReproducibleReduceTest, SimpleSum) {
    xxxxx-1ing::Communicator<std::vector, xxxxx-1ing::plugin::ReproducibleReducePlugin> full_comm;
    ...
    auto repr_comm = comm.make_reproducible_comm<double>(
        xxxxx-1ing::send_counts(distr.send_counts),
        xxxxx-1ing::recv_displs(distr.displs)
    );

    double sum = repr_comm.reproducible_reduce(xxxxx-1ing::send_buf(local_a), xxxxx-1ing::op(xxxxx-1ing::ops::plus<>()));
    EXPECT_EQ(sum, (1e3 + epsilon) + (epsilon / 2 + epsilon / 2));
}

template <typename F>
void with_comm_size_n(
    xxxxx-1ing::Communicator<std::vector, xxxxx-1ing::plugin::ReproducibleReducePlugin> const& comm, size_t comm_size, F f
) {
...
TEST(ReproducibleReduceTest, WorksWithNonzeroRoot) {
    ...
    with_comm_size_n(full_comm, 2, [&distribution, &array](auto comm) {
        auto repr_comm = comm.template make_reproducible_comm<double>(
            xxxxx-1ing::send_counts(distribution.send_counts),
            xxxxx-1ing::recv_displs(distribution.displs)
        );

        auto local_array = scatter_array(comm, array, distribution);

        double result =
            repr_comm.reproducible_reduce(xxxxx-1ing::send_buf(local_array), xxxxx-1ing::op(xxxxx-1ing::ops::plus<>{}));
...
TEST(ReproducibleReduceTest, Fuzzing) {
    xxxxx-1ing::Communicator<std::vector, xxxxx-1ing::plugin::ReproducibleReducePlugin> comm;
    comm.barrier();
    ...
    comm.bcast_single(xxxxx-1ing::send_recv_buf(seed));
    ...
    for (auto i = 0U; i < NUM_ARRAYS; ++i) {
        ...
        with_comm_size_n(comm, 1, [&reference_result, &data_array](auto comm_) {
            ...
            reference_result =
                repr_comm.reproducible_reduce(xxxxx-1ing::send_buf(data_array), xxxxx-1ing::op(xxxxx-1ing::ops::plus<>{}));
        ...
        for (auto j = 0U; j < NUM_DISTRIBUTIONS; ++j) {
            ...
            with_comm_size_n(comm, ranks, [&distribution, &data_array, &reference_result, &checks, &ranks](auto comm_) {
                ...
                double computed_result =
                    repr_comm.reproducible_reduce(xxxxx-1ing::send_buf(local_arr), xxxxx-1ing::op(xxxxx-1ing::ops::plus<>{}));
...
TEST(ReproducibleReduceTest, ReproducibleResults) {
    ...
    with_comm_size_n(comm, 1, [&reference_result, v_size, &v](auto sub_comm) {
        auto repr_comm = sub_comm.template make_reproducible_comm<double>(
            xxxxx-1ing::send_counts({xxxxx-1ing::asserting_cast<int>(v_size)}),
            xxxxx-1ing::recv_displs({0})
        );
        reference_result =
            repr_comm.template reproducible_reduce(xxxxx-1ing::send_buf(v), xxxxx-1ing::op(xxxxx-1ing::ops::plus<double>{}));
    });

    comm.bcast_single(xxxxx-1ing::send_recv_buf(reference_result));
    ...
    for (auto i = 2U; i <= comm.size(); ++i) {
        with_comm_size_n(comm, i, [&v, i, reference_result](auto subcomm) {
            ...
            subcomm.scatterv(
                xxxxx-1ing::send_buf(v),
                xxxxx-1ing::recv_buf<xxxxx-1ing::BufferResizePolicy::resize_to_fit>(local_v),
                xxxxx-1ing::send_counts(distr.send_counts),
                xxxxx-1ing::send_displs(distr.displs)
            );

            double const result =
                reprcomm.reproducible_reduce(xxxxx-1ing::send_buf(local_v), xxxxx-1ing::op(xxxxx-1ing::ops::plus<double>{}));
...
TEST(ReproducibleReduceTest, ErrorChecking) {
    ...
    with_comm_size_n(comm, 3, [](auto sub_comm) {
        ...
        EXPECT_KASSERT_FAILS(
            sub_comm.template make_reproducible_comm<double>(
                xxxxx-1ing::send_counts({5, 5, 5, 5}),
                xxxxx-1ing::recv_displs({0, 5, 10, 15})
            ),
            ""
        );
        ...
        EXPECT_KASSERT_FAILS(
            sub_comm.template make_reproducible_comm<double>(
                xxxxx-1ing::send_counts({5, 5, 5}),
                xxxxx-1ing::recv_displs({5, 10, 15})
            ),
            ""
        );
        ...
        EXPECT_KASSERT_FAILS(
            sub_comm.template make_reproducible_comm<double>(
                xxxxx-1ing::send_counts({5, 5, 5}),
                xxxxx-1ing::recv_displs({0, 10, 15})
            ),
            ""
        );
        ...
        EXPECT_KASSERT_FAILS(
            sub_comm.template make_reproducible_comm<double>(
                xxxxx-1ing::send_counts(std::vector<int>()),
                xxxxx-1ing::recv_displs(std::vector<int>())
            ),
            ""
        );
    });
...
TEST(ReproducibleReduceTest, OtherOperations) {
    ...
    auto const max_val =
        repr_comm.reproducible_reduce(xxxxx-1ing::send_buf(local_array), xxxxx-1ing::op(xxxxx-1ing::ops::max<>{}));
    EXPECT_EQ(max_val, 7.0);

    auto const min_val =
        repr_comm.reproducible_reduce(xxxxx-1ing::send_buf(local_array), xxxxx-1ing::op(xxxxx-1ing::ops::min<>{}));
    EXPECT_EQ(min_val, 1.0);

    auto const product =
        repr_comm.reproducible_reduce(xxxxx-1ing::send_buf(local_array), xxxxx-1ing::op(xxxxx-1ing::ops::multiplies<>{}));
    ...
    auto result = repr_comm.reproducible_reduce(
        xxxxx-1ing::send_buf(local_array),
        xxxxx-1ing::op(std::move(add_plus_42_lambda), xxxxx-1ing::ops::commutative)
    );
    ...
    auto subtracted = repr_comm.reproducible_reduce(
        xxxxx-1ing::send_buf(local_array),
        xxxxx-1ing::op([](auto const& lhs, auto const& rhs) { return lhs - rhs; }, xxxxx-1ing::ops::non_commutative)
    );
    ...
    auto plus3_result =
        repr_comm.reproducible_reduce(xxxxx-1ing::send_buf(local_array), xxxxx-1ing::op(Plus3{}, xxxxx-1ing::ops::commutative));
    EXPECT_EQ(plus3_result, array[0] + array[1] + array[2] + array[3] + array[4] + 4 * 3);

    // function pointer
    auto multiply_ptr =
        repr_comm.reproducible_reduce(xxxxx-1ing::send_buf(local_array), xxxxx-1ing::op(multiply, xxxxx-1ing::ops::commutative));
...
TEST(ReproducibleReduceTest, Microbenchmark) {
    ...
    for (auto i = 0U; i < repetitions; ++i) {
        result = repr_comm.reproducible_reduce(xxxxx-1ing::send_buf(local_array), xxxxx-1ing::op(xxxxx-1ing::ops::plus<>{}));
        timings.push_back(std::chrono::system_clock::now());
    }
