...
TEST(RequestPoolTest, wait_all) {
    ...
    for (auto& op: ops) {
        values.emplace_back();
        op.start_op(xxxxx-1ing::request(pool.get_request()), xxxxx-1ing::tag(42 + i), recv_buf(values.back()));
        i++;
    }
...
TEST(RequestPoolTest, wait_all_statuses_out_own_container_as_default) {
    using namespace ::testing;
    xxxxx-1ing::RequestPool<NonCopyableOwnContainer> pool;
    std::vector<DummyNonBlockingOperation>        ops(5);
    std::vector<int>                              values;
    values.reserve(5);
    int i = 0;
...
TEST(RequestPoolTest, test_all) {
    ...
    int                       val1;
    int                       val2;
    op1.start_op(xxxxx-1ing::request(pool.get_request()), xxxxx-1ing::tag(42), recv_buf(val1));
    op2.start_op(xxxxx-1ing::request(pool.get_request()), xxxxx-1ing::tag(43), recv_buf(val2));
