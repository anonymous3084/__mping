...
TEST(ExscanTest, exscan_single) {
    ...
    auto result = comm.exscan_single(send_buf(input), op(xxxxx-1ing::ops::plus<>{}));
...
TEST(ExscanTest, scan_single_with_temporary) {
    ...
    auto result          = comm.exscan_single(send_buf(42), op(xxxxx-1ing::ops::plus<>{}));
    int  expected_result = comm.rank_signed() * 42;
    EXPECT_EQ(result, expected_result);
}
...
TEST(ExscanTest, exscan_single_values_on_rank_0) {
    ...
    int result          = comm.exscan_single(send_buf(input), op(xxxxx-1ing::ops::plus<>{}), values_on_rank_0(0));
    int expected_result = comm.rank_signed() * 42;
    EXPECT_EQ(result, expected_result);
}
...
TEST(ExscanTest, exscan_single_vector_of_size_1) {
    ...
    auto result = comm.exscan_single(send_buf(input.front()), op(xxxxx-1ing::ops::plus<>{}));
...
TEST(ExscanTest, no_receive_buffer) {
    ...
    auto result = comm.exscan(send_buf(input), op(xxxxx-1ing::ops::plus<>{}));
...
TEST(ExscanTest, with_receive_buffer) {
    ...
    comm.exscan(send_buf(input), op(xxxxx-1ing::ops::plus<>{}), recv_buf<resize_to_fit>(result));
    EXPECT_EQ(result.size(), 2);
...
TEST(ExscanTest, with_receive_buffer_and_explicit_send_recv_count) {
    ...
    comm.exscan(send_buf(input), op(xxxxx-1ing::ops::plus<>{}), send_recv_count(1), recv_buf<resize_to_fit>(result));
...
TEST(ExscanTest, recv_buffer_is_given_and_larger_than_required) {
    ...
    }
    {
        ...
        comm.exscan(send_buf(input), op(xxxxx-1ing::ops::plus<>{}), recv_buf<grow_only>(result));
        EXPECT_THAT(result, ElementsAre(comm.rank_signed(), -1));
    }
    {
        std::vector<int> result(2, -1);
        // recv buffer will not be resizes as resize policy is no resize
        comm.exscan(send_buf(input), op(xxxxx-1ing::ops::plus<>{}), recv_buf<no_resize>(result));
        EXPECT_THAT(result, ElementsAre(comm.rank_signed(), -1));
    }
}
...
TEST(ExscanTest, recv_buffer_is_given_and_smaller_than_required) {
...
#if KASSERT_ENABLED(xxxxx-1_ASSERTION_LEVEL_NORMAL)
    {
        std::vector<int> result;
        // recv buffer will not be resizes as resize policy is no resize
        EXPECT_KASSERT_FAILS(comm.exscan(send_buf(input), op(xxxxx-1ing::ops::plus<>{}), recv_buf<no_resize>(result)), "");
    }
#endif
}
...
TEST(ExscanTest, builtin_op_on_non_builtin_type) {
    ...
    auto result =
        comm.exscan(send_buf(input), op(xxxxx-1ing::ops::plus<>{}, xxxxx-1ing::ops::commutative), values_on_rank_0(MyInt{0}));
    EXPECT_EQ(result.size(), 2);
...
TEST(ExscanTest, identity_not_auto_deducible_and_no_values_on_rank_0_provided) {
    ...
    auto result = comm.exscan(send_buf(input), op(xxxxx-1ing::ops::plus<>{}, xxxxx-1ing::ops::commutative));
    EXPECT_EQ(result.size(), 2);
    std::vector<MyInt> expected_result = {((comm.rank_signed() - 1) * comm.rank_signed()) / 2, comm.rank_signed() * 42};
...
TEST(ExscanTest, non_identity_values_on_rank_0) {
    ...
    auto result = comm.exscan(send_buf(input), op(xxxxx-1ing::ops::plus<>{}), values_on_rank_0(1337));
...
TEST(ExscanTest, custom_operation_on_builtin_type) {
    ...
    { // use function ptr
        auto result = comm.exscan(
            send_buf(input),
            op(add_plus_42_function, xxxxx-1ing::ops::commutative),
    ...
    { // use lambda
        auto result = comm.exscan(
            send_buf(input),
            op(add_plus_42_lambda, xxxxx-1ing::ops::commutative),
            values_on_rank_0({0, 1, 2})
        );
    ...
    { // use function object
        ...
        auto result =
            comm.exscan(send_buf(input), op(MySum42{}, xxxxx-1ing::ops::commutative), values_on_rank_0({0, 1, 2}));
...
TEST(ExscanTest, custom_operation_on_builtin_type_non_commutative) {
    ...
    auto result = comm.exscan(send_buf(input), op(get_right, xxxxx-1ing::ops::non_commutative), values_on_rank_0(0));
...
TEST(ExscanTest, default_container_type) {
    ...
    OwnContainer<int> result = comm.exscan(send_buf(input), op(xxxxx-1ing::ops::plus<>{}));
}
...
TEST(ExscanTest, send_recv_count_is_part_of_result_object) {
    ...
    std::vector<int> data{0, 1};
    auto             result =
        comm.exscan(send_buf(data), send_recv_count_out(), op(xxxxx-1ing::ops::plus<>{}), values_on_rank_0({0, 0}));
...
TEST(ExscanTest, structured_bindings_explicit_recv_buffer) {
    ...
    auto [send_recv_count] =
        comm.exscan(send_buf(input), send_recv_count_out(), op(xxxxx-1ing::ops::plus<>{}), recv_buf(recv_buffer));
...
TEST(ExscanTest, structured_bindings_implicit_recv_buffer) {
    ...
    auto [recv_buffer, send_recv_type, send_recv_count] =
        comm.exscan(send_recv_type_out(), send_buf(input), send_recv_count_out(), op(xxxxx-1ing::ops::plus<>{}));
    EXPECT_THAT(recv_buffer, ElementsAre(comm.rank() * 42, comm.rank()));
    EXPECT_EQ(send_recv_count, 2);
...
TEST(ExscanTest, inplace_basic) {
    ...
    comm.exscan(send_recv_buf(data), op(xxxxx-1ing::ops::plus<>{}));
...
TEST(ExscanTest, inplace_out_parameters) {
    ...
    std::vector<int> data = {42, 1};
    auto [count, type] =
        comm.exscan(send_recv_buf(data), op(xxxxx-1ing::ops::plus<>{}), send_recv_count_out(), send_recv_type_out());
    EXPECT_EQ(count, 2);
...
TEST(ExscanTest, inplace_rvalue_buffer) {
    ...
    std::vector<int> data = {42, 1};
    data                  = comm.exscan(send_recv_buf(std::move(data)), op(xxxxx-1ing::ops::plus<>{}));
    EXPECT_THAT(data, ElementsAre(comm.rank() * 42, comm.rank()));
}
...
TEST(ExscanTest, inplace_explicit_count) {
    ...
    std::vector<int> data = {42, 1};
    comm.exscan(send_recv_buf(data), send_recv_count(1), op(xxxxx-1ing::ops::plus<>{}));
...
TEST(ExscanTest, inplace_explicit_count_resize) {
    ...
    std::vector<int> data = {42, 1};
    comm.exscan(send_recv_buf<resize_to_fit>(data), send_recv_count(1), op(xxxxx-1ing::ops::plus<>{}));
