...
TEST(AllreduceTest, allreduce_no_receive_buffer) {
    ...
    auto result = comm.allreduce(send_buf(input), op(xxxxx-1ing::ops::plus<>{}));
...
TEST(AllreduceTest, allreduce_with_receive_buffer) {
    ...
    comm.allreduce(send_buf(input), op(xxxxx-1ing::ops::plus<>{}), recv_buf<resize_to_fit>(result));
    EXPECT_EQ(result.size(), 2);
...
TEST(AllreduceTest, allreduce_with_receive_buffer_no_resize_and_explicit_send_recv_count) {
    ...
    comm.allreduce(send_buf(input), op(xxxxx-1ing::ops::plus<>{}), recv_buf<no_resize>(result), send_recv_count(1));
...
TEST(AllreduceTest, allreduce_with_receive_buffer_grow_only_and_explicit_send_recv_count) {
    ...
    comm.allreduce(send_buf(input), op(xxxxx-1ing::ops::plus<>{}), recv_buf<grow_only>(result), send_recv_count(1));
    EXPECT_THAT(result, ElementsAre(comm.size(), 42));
}

#if KASSERT_ENABLED(xxxxx-1_ASSERTION_LEVEL_LIGHT)
    ...
    EXPECT_KASSERT_FAILS(
        {
            comm.allreduce(
                ...
                recv_buf<xxxxx-1ing::BufferResizePolicy::no_resize>(result),
                send_recv_count(1)
            );
        },
        ""
...
TEST(AllreduceTest, allreduce_builtin_op_on_non_builtin_type) {
    ...
    auto result = comm.allreduce(send_buf(input), op(xxxxx-1ing::ops::plus<>{}, xxxxx-1ing::ops::commutative));
    EXPECT_EQ(result.size(), 2);
...
TEST(AllreduceTest, allreduce_custom_operation_on_builtin_type) {
    ...
    { // use function ptr
        auto result = comm.allreduce(send_buf(input), op(add_plus_42_function, xxxxx-1ing::ops::commutative));
    ...
    { // use lambda
        auto result = comm.allreduce(send_buf(input), op(add_plus_42_lambda, xxxxx-1ing::ops::commutative));
    ...
    { // use function object
        ...
        auto result = comm.allreduce(send_buf(input), op(MySum42{}, xxxxx-1ing::ops::commutative));
...
TEST(AllreduceTest, allreduce_custom_operation_on_builtin_type_non_commutative) {
    ...
    auto result = comm.allreduce(send_buf(input), op(get_right, xxxxx-1ing::ops::non_commutative));
...
TEST(AllreduceTest, allreduce_custom_operation_on_custom_type) {
    ...
    auto result = comm.allreduce(send_buf(input), op(my_op, xxxxx-1ing::ops::commutative));
...
TEST(AllreduceTest, allreduce_default_container_type) {
    ...
    OwnContainer<int> result = comm.allreduce(send_buf(input), op(xxxxx-1ing::ops::plus<>{}));
}
...
TEST(AllreduceTest, send_recv_type_is_out_parameter) {
    ...
    auto recv_buf = comm.allreduce(send_buf(data), send_recv_type_out(send_recv_type), op(xxxxx-1ing::ops::plus<>{}));
...
TEST(AllreduceTest, send_recv_type_part_of_result_object) {
    Communicator           comm;
    std::vector<int> const data{1};
    auto                   result = comm.allreduce(send_buf(data), send_recv_type_out(), op(xxxxx-1ing::ops::plus<>{}));
...
//         EXPECT_KASSERT_FAILS(
//             comm.allreduce(send_buf(input), op(xxxxx-1ing::ops::plus<>{})),
...
TEST(AllreduceTest, allreduce_single) {
    ...
    int const result = comm.allreduce_single(send_buf(input), op(xxxxx-1ing::ops::plus<>{}));
...
TEST(AllreduceTest, allreduce_single_with_temporary) {
    ...
    int const result = comm.allreduce_single(send_buf(comm.rank_signed()), op(xxxxx-1ing::ops::plus<>{}));
...
TEST(AllreduceTest, structured_bindings_implicit_recv_buffer) {
    ...
    std::vector<std::uint64_t> const expected_recv_buffer(2, comm.size() * (comm.size() - 1) / 2);
    {
        ...
        auto const [recv_buffer, send_recv_type, send_recv_count] =
            comm.allreduce(send_recv_type_out(), send_recv_count_out(), send_buf(values), op(xxxxx-1ing::ops::plus<>{}));
...
TEST(AllreduceTest, inplace_basic) {
    ...
    comm.allreduce(send_recv_buf(values), op(xxxxx-1ing::ops::plus<>{}));
...
TEST(AllreduceTest, inplace_out_parameters) {
    ...
    std::vector<int> values = {comm.rank_signed(), comm.rank_signed()};
    auto [count, type] =
        comm.allreduce(send_recv_buf(values), op(xxxxx-1ing::ops::plus<>{}), send_recv_count_out(), send_recv_type_out());
...
TEST(AllreduceTest, inplace_rvalue_buffer) {
    ...
    std::vector<int> values = {comm.rank_signed(), comm.rank_signed()};
    auto             result = comm.allreduce(send_recv_buf(std::move(values)), op(xxxxx-1ing::ops::plus<>()));
...
TEST(AllreduceTest, inplace_explicit_count) {
    ...
    comm.allreduce(send_recv_buf(values), op(xxxxx-1ing::ops::plus<>{}), send_recv_count(1));
