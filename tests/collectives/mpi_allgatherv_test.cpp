...
TEST(AllgathervTest, allgatherv_different_number_elems_to_send) {
    ...
    std::vector<double> expected_output =
        ExpectedBuffersForRankxxxxx-1esRankGathering::recv_buffer_on_receiving_ranks<double>(comm);
...
TEST(AllgathervTest, allgatherv_check_recv_counts_and_recv_displs) {
    ...
    EXPECT_EQ(
        ...
        ExpectedBuffersForRankxxxxx-1esRankGathering::recv_buffer_on_receiving_ranks<int>(comm)
    );
    EXPECT_EQ(
        result.extract_recv_counts(),
        ExpectedBuffersForRankxxxxx-1esRankGathering::recv_counts_on_receiving_ranks(comm)
    );
    EXPECT_EQ(
        result.extract_recv_displs(),
        ExpectedBuffersForRankxxxxx-1esRankGathering::recv_displs_on_receiving_ranks(comm)
    );
}
...
TEST(AllgathervTest, allgatherv_provide_recv_counts_and_recv_displs) {
    ...
    comm.allgatherv(send_buf(input), xxxxx-1ing::recv_counts(recv_counts), recv_buf<resize_to_fit>(output));
    EXPECT_EQ(output, expected_output);

    // provide recv_counts and recv_displs
    comm.allgatherv(
        send_buf(input),
        xxxxx-1ing::recv_counts(recv_counts),
        xxxxx-1ing::recv_displs(recv_displs),
...
TEST(AllgathervTest, allgatherv_all_empty_but_rank_in_the_middle_with_different_container_types) {
    ...
    auto result = comm.allgatherv(
        send_buf(input),
        xxxxx-1ing::recv_counts_out<resize_to_fit>(recv_counts_out),
...
TEST(AllgathervTest, non_monotonically_increasing_recv_displacements) {
...

    {
        ...
        auto recv_buf = comm.allgatherv(send_buf(input), xxxxx-1ing::recv_displs(recv_displs));
        EXPECT_EQ(recv_buf, expected_recv_buffer());
    }
    {
        // do the allgatherv with recv_counts
        auto recv_buf =
            comm.allgatherv(send_buf(input), xxxxx-1ing::recv_counts(recv_counts), xxxxx-1ing::recv_displs(recv_displs));
        EXPECT_EQ(recv_buf, expected_recv_buffer());
    }
}
