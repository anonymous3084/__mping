...
#include "xxxxx-1ing/collectives/scan.hpp"
...
TEST(ScanTest, scan_single) {
    ...
    auto result = comm.scan_single(send_buf(input), op(xxxxx-1ing::ops::plus<>{}));
...
TEST(ScanTest, scan_single_with_temporary) {
    ...
    auto result          = comm.scan_single(send_buf(42), op(xxxxx-1ing::ops::plus<>{}));
    int  expected_result = (comm.rank_signed() + 1) * 42;
    EXPECT_EQ(result, expected_result);
}
...
TEST(ScanTest, scan_single_vector_of_size_1) {
    ...
    auto result = comm.scan_single(send_buf(input.front()), op(xxxxx-1ing::ops::plus<>{}));
...
TEST(ScanTest, scan_explicit_send_recv_count_smaller_than_send_buffer_size) {
    ...
    auto recv_buf = comm.scan(send_buf(input), send_recv_count(2), op(xxxxx-1ing::ops::plus<>{}));
...
TEST(ScanTest, scan_explicit_send_recv_count_out_value_not_taken_into_account) {
    ...
    auto recv_buf = comm.scan(send_buf(input), send_recv_count_out(send_recv_count), op(xxxxx-1ing::ops::plus<>{}));
...
TEST(ScanTest, scan_no_receive_buffer) {
    ...
    auto result = comm.scan(send_buf(input), op(xxxxx-1ing::ops::plus<>{}));
...
TEST(ScanTest, scan_with_receive_buffer) {
    ...
    comm.scan(send_buf(input), op(xxxxx-1ing::ops::plus<>{}), recv_buf<resize_to_fit>(result));
    EXPECT_EQ(result.size(), 2);
...
TEST(ScanTest, scan_builtin_op_on_non_builtin_type) {
    ...
    auto result = comm.scan(send_buf(input), op(xxxxx-1ing::ops::plus<>{}, xxxxx-1ing::ops::commutative));
    EXPECT_EQ(result.size(), 2);
...
TEST(ScanTest, scan_custom_operation_on_builtin_type) {
    ...
    { // use function ptr
        auto result = comm.scan(send_buf(input), op(add_plus_42_function, xxxxx-1ing::ops::commutative));
    ...
    { // use lambda
        auto result = comm.scan(send_buf(input), op(add_plus_42_lambda, xxxxx-1ing::ops::commutative));
    ...
    { // use lambda inline
        auto result = comm.scan(
            send_buf(input),
            op([](auto const& lhs, auto const& rhs) { return lhs + rhs + 42; }, xxxxx-1ing::ops::commutative)
        );
    ...
    { // use function object
        ...
        auto result = comm.scan(send_buf(input), op(MySum42{}, xxxxx-1ing::ops::commutative));
...
TEST(ScanTest, scan_custom_operation_on_builtin_type_non_commutative) {
    ...
    auto result = comm.scan(send_buf(input), op(get_right, xxxxx-1ing::ops::non_commutative));
...
TEST(ScanTest, scan_custom_operation_on_custom_type) {
    ...
    auto result = comm.scan(send_buf(input), op(my_op, xxxxx-1ing::ops::commutative));
...
TEST(ScanTest, scan_default_container_type) {
    ...
    OwnContainer<int> result = comm.scan(send_buf(input), op(xxxxx-1ing::ops::plus<>{}));
}
...
TEST(ScanTest, single_element_with_given_recv_buf_bigger_than_required) {
...

    {
        ...
        comm.scan(send_buf(input), recv_buf<resize_to_fit>(recv_buffer), op(xxxxx-1ing::ops::plus<>{}));
    ...
    }
    {
        ...
        std::vector<int> recv_buffer(2, -1);
        comm.scan(send_buf(input), recv_buf<grow_only>(recv_buffer), op(xxxxx-1ing::ops::plus<>{}));
        EXPECT_THAT(recv_buffer, ElementsAre(expected_recv_value, -1));
    }
    {
        // recv buffer will not be resized as the policy is no_resize
        std::vector<int> recv_buffer(2, -1);
        comm.scan(send_buf(input), recv_buf<no_resize>(recv_buffer), op(xxxxx-1ing::ops::plus<>{}));
        EXPECT_THAT(recv_buffer, ElementsAre(expected_recv_value, -1));
    }
    {
        // recv buffer will not be resized as the policy is no_resize (default)
        std::vector<int> recv_buffer(2, -1);
        comm.scan(send_buf(input), recv_buf(recv_buffer), op(xxxxx-1ing::ops::plus<>{}));
        EXPECT_THAT(recv_buffer, ElementsAre(expected_recv_value, -1));
    }
}
...
TEST(ScanTest, send_recv_count_is_out_parameter) {
    ...
    int              send_recv_count = -1;
    auto result = comm.scan(send_buf(data), send_recv_count_out(send_recv_count), op(xxxxx-1ing::ops::plus<>{}));
...
TEST(ScanTest, send_recv_count_is_part_of_result_object) {
    Communicator     comm;
    std::vector<int> data{0, 1};
    auto             result = comm.scan(send_buf(data), send_recv_count_out(), op(xxxxx-1ing::ops::plus<>{}));
...
TEST(ScanTest, send_recv_type_is_out_parameter) {
    ...
    auto             result =
        comm.scan(send_buf(data), send_recv_count(2), op(xxxxx-1ing::ops::plus<>{}), send_recv_type_out(send_recv_type));
...
TEST(ScanTest, send_recv_type_is_part_of_result_object) {
    Communicator     comm;
    std::vector<int> data{0, 1};
    auto result = comm.scan(send_buf(data), send_recv_count(2), op(xxxxx-1ing::ops::plus<>{}), send_recv_type_out());
...
TEST(ScanTest, custom_operation_on_custom_mpi_type) {
    ...
    comm.scan(
        ...
        op(my_op, xxxxx-1ing::ops::commutative),
...
TEST(ScanTest, structured_bindings_explicit_recv_buffer) {
    ...
    auto [send_recv_count] =
        comm.scan(send_buf(input), send_recv_count_out(), op(xxxxx-1ing::ops::plus<>{}), recv_buf(recv_buffer));
...
TEST(ScanTest, structured_bindings_implicit_recv_buffer) {
    ...
    auto [recv_buffer, send_recv_type, send_recv_count] =
        comm.scan(send_recv_type_out(), send_buf(input), send_recv_count_out(), op(xxxxx-1ing::ops::plus<>{}));
    EXPECT_THAT(recv_buffer, ElementsAre((comm.rank() + 1) * 42, (comm.rank() + 1)));
    EXPECT_EQ(send_recv_count, 2);
...
TEST(ScanTest, inplace_basic) {
    ...
    comm.scan(send_recv_buf(data), op(xxxxx-1ing::ops::plus<>{}));
...
TEST(ScanTest, inplace_out_parameters) {
    ...
    std::vector<int> data = {42, 1};
    auto [count, type] =
        comm.scan(send_recv_buf(data), send_recv_count_out(), send_recv_type_out(), op(xxxxx-1ing::ops::plus<>{}));
    EXPECT_EQ(count, 2);
...
TEST(ScanTest, inplace_rvalue_buffer) {
    ...
    auto result = comm.scan(send_recv_buf(std::vector<int>{42, 1}), op(xxxxx-1ing::ops::plus<>{}));
...
TEST(ScanTest, inplace_explicit_count) {
    ...
    std::vector<int> data = {42, 1};
    comm.scan(send_recv_buf(data), send_recv_count(1), op(xxxxx-1ing::ops::plus<>{}));
...
TEST(ScanTest, inplace_explicit_count_resize) {
    ...
    std::vector<int> data = {42, 1};
    comm.scan(send_recv_buf<resize_to_fit>(data), send_recv_count(1), op(xxxxx-1ing::ops::plus<>{}));
