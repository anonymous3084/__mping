...
TEST(ReduceTest, reduce_no_receive_buffer) {
    ...
    auto result = comm.reduce(send_buf(input), op(xxxxx-1ing::ops::plus<>{}));
    ...
    result = comm.reduce(send_buf(input), op(xxxxx-1ing::ops::plus<>{}));
    if (comm.is_root()) {
    ...
    for (size_t i = 0; i < comm.size(); ++i) {
        result = comm.reduce(send_buf(input), op(xxxxx-1ing::ops::plus<>{}), root(i));
        if (comm.rank() == i) {
            EXPECT_EQ(comm.root(), comm.size() - 1);
            EXPECT_EQ(result.size(), 2);
            EXPECT_EQ(result, expected_result);
...
TEST(ReduceTest, reduce_single_element_explicit_receive_buffer_bool) {
    ...
    comm.reduce(send_buf(input), recv_buf<xxxxx-1ing::BufferResizePolicy::resize_to_fit>(result), op(ops::logical_or<>{}));
...
TEST(ReduceTest, reduce_with_receive_buffer) {
    ...
    comm.reduce(
        ...
        recv_buf<xxxxx-1ing::BufferResizePolicy::resize_to_fit>(result)
    );
...
TEST(ReduceTest, reduce_with_receive_buffer_grow_only_and_explicit_send_recv_count) {
    ...
    comm.reduce(
        send_buf(input),
        op(xxxxx-1ing::ops::plus<>{}),
        recv_buf<xxxxx-1ing::BufferResizePolicy::grow_only>(result),
        send_recv_count(1)
    );
...
TEST(ReduceTest, reduce_builtin_op_on_non_builtin_type) {
    ...
    auto               result = comm.reduce(send_buf(input), op(xxxxx-1ing::ops::plus<>{}, xxxxx-1ing::ops::commutative));
    if (comm.is_root()) {
        EXPECT_EQ(result.size(), 2);
...
TEST(ReduceTest, reduce_custom_operation_on_builtin_type) {
    ...
    // use function ptr
    auto result = comm.reduce(send_buf(input), op(add_plus_42_function, xxxxx-1ing::ops::commutative));
    ...
    // use lambda
    result = comm.reduce(send_buf(input), op(add_plus_42_lambda, xxxxx-1ing::ops::commutative));
    ...
    result = comm.reduce(send_buf(input), op(MySum42{}, xxxxx-1ing::ops::commutative));
...
TEST(ReduceTest, reduce_custom_operation_on_builtin_type_non_commutative) {
    ...
    auto result = comm.reduce(send_buf(input), op(get_right, xxxxx-1ing::ops::non_commutative));
...
TEST(ReduceTest, reduce_custom_operation_on_custom_type) {
    ...
    auto result = comm.reduce(send_buf(input), op(my_op, xxxxx-1ing::ops::commutative));
...
TEST(ReduceTest, reduce_default_container_type) {
    ...
    OwnContainer<int> result = comm.reduce(send_buf(input), op(xxxxx-1ing::ops::plus<>{}));
}
...
TEST(ReduceTest, send_recv_type_is_out_parameter) {
    ...
    int const              root_rank = 0;
    auto                   recv_buf =
        comm.reduce(send_buf(data), send_recv_type_out(send_type), op(xxxxx-1ing::ops::plus<>{}), root(root_rank));
...
TEST(ReduceTest, send_type_part_of_result_object) {
    ...
    std::vector<int> const data{1};
    int const              root_rank = 0;
    auto result = comm.reduce(send_buf(data), send_recv_type_out(), op(xxxxx-1ing::ops::plus<>{}), root(root_rank));
...
//         EXPECT_KASSERT_FAILS(
//             comm.reduce(send_buf(value), op(xxxxx-1ing::ops::plus<>{}), root(comm.rank())),
...
TEST(ReduceTest, reduce_single) {
    ...
    std::optional<int> result = comm.reduce_single(send_buf(input), op(xxxxx-1ing::ops::plus<>{}));
...
TEST(ReduceTest, reduce_single_with_temporary) {
    Communicator       comm;
    std::optional<int> result = comm.reduce_single(send_buf(comm.rank_signed()), op(xxxxx-1ing::ops::plus<>{}));
...
TEST(ReduceTest, reduce_single_with_root_param) {
    ...
    std::optional<int> result = comm.reduce_single(send_buf(input), xxxxx-1ing::root(root), op(xxxxx-1ing::ops::plus<>{}));
...
TEST(ReduceTest, structured_bindings_implicit_recv_buffer) {
    ...
    std::vector<std::uint64_t> const expected_recv_buffer_on_root(2, comm.size() * (comm.size() - 1) / 2);
    {
        ...
        auto const [recv_buffer, send_recv_type, send_recv_count] =
            comm.reduce(send_recv_type_out(), send_recv_count_out(), send_buf(values), op(xxxxx-1ing::ops::plus<>{}));
