...
int main(int /*argc*/, char** /*argv*/) {
    ...
    comm.gather(send_buf(input), send_type(MPI_INT));
#elif defined(RECV_TYPE_GIVEN_BUT_NO_RECV_COUNT)
    comm.gather(send_buf(input), recv_type(MPI_INT), recv_buf<no_resize>(recv_buffer));
#elif defined(RECV_TYPE_GIVEN_BUT_RESIZE_POLICY_IS_RESIZE_TO_FIT)
    comm.gather(send_buf(input), recv_type(MPI_INT), recv_count(1), recv_buf<resize_to_fit>(recv_buffer));
#elif defined(RECV_TYPE_GIVEN_BUT_RESIZE_POLICY_IS_GROW_ONLY)
    comm.gather(send_buf(input), recv_type(MPI_INT), recv_count(1), recv_buf<grow_only>(recv_buffer));
#else
