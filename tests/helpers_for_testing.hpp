...
template <xxxxx-1ing::internal::ParameterType _parameter_type>
struct Argument {
    static constexpr xxxxx-1ing::internal::ParameterType parameter_type = _parameter_type;
    Argument(int i) : _i{i} {}
    int _i;
};
...
struct DummyNonBlockingOperation {
    ...
    ///- \c request: The request object to use. Defaults to internally constructed \c xxxxx-1ing::request() which is return
    /// to the user.
    ///- \c recv_buf: The receive buffer to use. Defaults to \c xxxxx-1ing::recv_buf(alloc_new<std::vector<int>>).
    ...
    auto start_op(Args... args) {
        ...
        xxxxx-1_CHECK_PARAMETERS(
            ...
            xxxxx-1_OPTIONAL_PARAMETERS(tag, request, recv_buf)
        );
        using default_recv_buf_type = decltype(xxxxx-1ing::recv_buf(alloc_new<std::vector<int>>));
        auto&& recv_buf =
        ...
        return xxxxx-1ing::internal::make_nonblocking_result<std::tuple<RecvBufType, RequestType>>(
            std::move(recv_buf),
...
std::vector<MPI_Datatype> possible_mpi_datatypes() noexcept {
    ...
    if constexpr (std::is_same_v<T_no_const, xxxxx-1ing::kabool>) {
        possible_mpi_datatypes.push_back(MPI_CXX_BOOL);
    }
