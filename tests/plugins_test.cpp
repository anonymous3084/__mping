...
class Send42Plugin : public xxxxx-1ing::plugin::PluginBase<Comm, DefaultContainerType, Send42Plugin> {
public:
    ...
    void send_42(size_t target_rank) {
        ...
        this->to_communicator().send(xxxxx-1ing::send_buf(send_buf), xxxxx-1ing::destination(target_rank));
    }
};

TEST(PluginsTest, additional_function) {
    if (xxxxx-1ing::comm_world().size() < 2) {
        return;
    }
    ...
    xxxxx-1ing::Communicator<std::vector, Send42Plugin> comm;
...
template <typename Comm, template <typename...> typename DefaultContainerType>
class AlternativeAllreduce : public xxxxx-1ing::plugin::PluginBase<Comm, DefaultContainerType, AlternativeAllreduce> {
public:
    /// @brief Has the same functionality as `xxxxx-1ing::Communicator::allreduce` with the exception that a `recv_buf`
    ...
    void allreduce(Args... args) {
        xxxxx-1_CHECK_PARAMETERS(
            Args,
            xxxxx-1_REQUIRED_PARAMETERS(send_buf, op, recv_buf),
        ...
        for (int i = 1; i < this->to_communicator().size_signed(); ++i) {
            this->to_communicator().reduce(xxxxx-1ing::root(i), std::move(args)...);
        }
    }
};

/// @brief Create a new Communicator class that uses the alternative allreduce implementation.
class MyComm : public xxxxx-1ing::Communicator<std::vector, AlternativeAllreduce, Send42Plugin> {
public:
...
TEST(PluginsTest, replace_implementation) {
    ...
    // First, a quick example of how NOT to overwrite an existing function:
    {
        ...
        xxxxx-1ing::Communicator<std::vector, AlternativeAllreduce> faultyComm;
        ...
        faultyComm.allreduce(
            xxxxx-1ing::send_buf(input),
            xxxxx-1ing::op(xxxxx-1ing::ops::plus<>{}),
...
public:
    ...
    class SendDefaultConstructed
        : public xxxxx-1ing::plugin::PluginBase<Comm, DefaultContainterType, SendDefaultConstructed> {
    public:
...
TEST(PluginsTest, additional_function_with_double_template) {
    ...
    xxxxx-1ing::Communicator<std::vector, SendDefaultConstructedOuterClass<double>::SendDefaultConstructed> comm;
    if (comm.size() < 2) {
        return;
    }
...
template <typename Comm, template <typename...> typename DefaultContainterType>
class IncrementalSend : public xxxxx-1ing::plugin::PluginBase<Comm, DefaultContainterType, IncrementalSend> {
public:
    ...
    void send_incremental(size_t target_rank) {
        // Use the built-in send function.
        // Uses the `to_communicator` function of `PluginBase` to cast itself to `Comm`.
        this->to_communicator().send(xxxxx-1ing::send_buf(_send_buf), xxxxx-1ing::destination(target_rank));
        ++_send_buf;
    }
...
template <typename Comm, template <typename...> typename DefaultContainerType>
class DecrementalSend : public xxxxx-1ing::plugin::PluginBase<Comm, DefaultContainerType, DecrementalSend> {
public:
    /// @brief Default constructor initializing the integer sent to 42.
...
TEST(PluginsTest, plugins_with_data_member) {
    // Create a new communicator. The first template argument is the default container type (has to be provided when
    // using plugins). The following template arguments are plugin classes.
    xxxxx-1ing::Communicator<std::vector, IncrementalSend, DecrementalSend> comm;
    if (comm.size() < 2) {
        return;
    }
