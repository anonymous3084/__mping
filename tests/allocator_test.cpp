...
#include "xxxxx-1ing/allocator.hpp"
...
TEST_F(AllocatorTest, simple_allocation) {
    {
        xxxxx-1ing::MPIAllocator<std::byte> alloc;
    ...
    {
        xxxxx-1ing::MPIAllocator<double> alloc;
...
TEST_F(AllocatorTest, custom_type_allocation) {
    xxxxx-1ing::MPIAllocator<double>                                       double_alloc;
...
TEST_F(AllocatorTest, vector_allocation) {
    std::vector<double, xxxxx-1ing::MPIAllocator<double>> v;
    EXPECT_EQ(AllocatorTest::allocated_memory, 0);
    v.push_back(4.12);
    v.push_back(135.134);
    v.push_back(351.123);
    ...
    v.shrink_to_fit();
    EXPECT_EQ(v.capacity() * sizeof(double), AllocatorTest::allocated_memory);
    std::vector<double, xxxxx-1ing::MPIAllocator<double>>{}.swap(v);
